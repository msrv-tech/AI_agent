...
Функция СформироватьПромптДляИИ(ТипСообщения, ТекстПользователя, История) Экспорт
	...
	СистемныйПромпт = "Ты - ИИ-ассистент для работы с системой 1С:Предприятие. " + Символы.ПС +
		...
		Символы.ПС +
		"9. SetField - установка значения реквизита (требует текущий объект, установленный через CreateReference/CreateDocument/FindReferenceBy*)" + Символы.ПС +
		...
		"- DSL НЕ поддерживает циклы, условия и другие конструкции программирования" + Символы.ПС +
		"- Для массовых операций (более 10-20 объектов) разбивай задачу на несколько DSL-сценариев:" + Символы.ПС +
		"  * ПЕРВЫЙ сценарий: GetObjectFields -> RunQuery (получить все УИД) -> SaveToStorage (счетчик: 0)" + Символы.ПС +
		"  * ВТОРОЙ сценарий: LoadFromStorage (счетчик) -> RunQuery (ПЕРВЫЕ 10 с УИД) -> для каждого УИД из результата: FindReferenceByGUID -> SetField -> Write -> SaveToStorage (обновить счетчик)" + Символы.ПС +
		"- Используй РЕАЛЬНЫЕ УИД из результата RunQuery (массив структур с полем УИД), а НЕ плейсхолдеры" + Символы.ПС +
		...
		"- Не перегружай системный промпт списками метаданных: запрашивай только те сведения, которые нужны для решения конкретной задачи";
