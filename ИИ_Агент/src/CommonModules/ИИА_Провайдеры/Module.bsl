...
Функция СформироватьПромптДляИИ(ТипСообщения, ТекстПользователя, История) Экспорт
	...
	СистемныйПромпт = "Ты - ИИ-ассистент для работы с системой 1С:Предприятие. " + Символы.ПС +
		...
		"     - collection (строка): имя ключа коллекции в контексте (обычно загруженного через LoadFromStorage)" + Символы.ПС +
		...
		"    Пример: {""action"": ""ForEach"", ""collection"": ""список_уид"", ""steps"": [{""action"": ""FindReferenceByGUID"", ""guid"": ""#(CurrentItem.УИД)""}, {""action"": ""SetField"", ""field_name"": ""ПометкаНаУдаление"", ""value"": true}, {""action"": ""Write""}]}" + Символы.ПС +
		Символы.ПС +
		"ВАЖНО:" + Символы.ПС +
		...
		"- Для массовых операций используй связку: GetObjectFields -> RunQuery -> SaveToStorage -> LoadFromStorage -> ForEach" + Символы.ПС +
		"  * ПЕРВЫЙ сценарий: GetObjectFields -> RunQuery (получить все ссылки или УИД) -> SaveToStorage (счетчик: 0)" + Символы.ПС +
		"  * ВТОРОЙ сценарий: LoadFromStorage (счетчик) -> RunQuery (ПЕРВЫЕ 10) -> для каждого элемента: SelectObject (по ссылке) или FindReferenceByGUID (по УИД) -> SetField -> Write -> SaveToStorage (обновить счетчик)" + Символы.ПС +
