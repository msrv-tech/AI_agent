...
Процедура ДобавитьСообщениеВДиалог(СсылкаДиалога, Автор, ТипСообщения, Текст, ТекстКода = "", Статус = "", UsageTokens = 0, СтатусDSL = Неопределено, ЗадачаВыполнена = Неопределено) Экспорт
	...
	// Дополнительная проверка: если статус не установлен, устанавливаем по умолчанию
	Если НЕ ЗначениеЗаполнено(НоваяСтрока.СтатусDSL) Тогда
		НоваяСтрока.СтатусDSL = Перечисления.ИИА_СтатусDSL.НеПрименяется;
	КонецЕсли;
...
Функция ОтправитьСообщениеСервера(СсылкаДиалога, ТипСообщения, ТекстПользователя) Экспорт
	...
	Если ОтветИИ.ТипОтвета = "DSL" И НЕ ПустаяСтрока(ОтветИИ.DSL) Тогда
		...
		// Обрабатываем результат выполнения DSL
		ОбработатьРезультатDSL(РезультатDSL, СсылкаДиалога, ТекстПользователя, Настройки, ОтветИИ.DSL, Истина, 0, 5);
...
//
Процедура ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога, ТекстЗадачиПользователя, РезультатDSL, Настройки)
	...
	Попытка
		...
		ПромптПроверки = "Проверь, выполнена ли задача пользователя и правильно ли заполнены созданные объекты." + Символы.ПС +
			"Задача пользователя: " + ТекстЗадачиПользователя + Символы.ПС +
			Символы.ПС +
			"Результат выполнения:" + Символы.ПС +
			ТекстРезультатов + Символы.ПС;
		
		Если НЕ ПустаяСтрока(ТекстДанныхОбъектов) Тогда
			ПромптПроверки = ПромптПроверки + 
				"Детальная информация о созданных объектах (реквизиты и их значения):" + Символы.ПС +
				ТекстДанныхОбъектов + Символы.ПС;
		КонецЕсли;
		
		ПромптПроверки = ПромптПроверки + 
			"Проверь:" + Символы.ПС +
			"1. Соответствует ли результат задаче пользователя" + Символы.ПС +
			"2. Правильно ли заполнены реквизиты созданных объектов" + Символы.ПС +
			"3. Все ли необходимые поля установлены" + Символы.ПС +
			Символы.ПС +
			"Ответь только 'ДА' или 'НЕТ' без дополнительных комментариев.";
		
		// Формируем историю для запроса (пустая таблица)
		История = ПреобразоватьМассивСообщенийВТаблицу(Новый Массив);
		
		// Вызываем ИИ для проверки
		ОтветПроверки = ИИА_Провайдеры.ВызватьИИ("Чат", ПромптПроверки, История, Настройки);
		
		// Анализируем ответ
		ЗадачаВыполнена = Ложь;
		Если НЕ ПустаяСтрока(ОтветПроверки.Текст) Тогда
		...
		// Обновляем признак выполнения задачи в последнем сообщении пользователя
		Если Диалог.Сообщения.Количество() > 0 Тогда
			...
			Пока Индекс >= 0 Цикл
				...
				Если СтрокаСообщения.Автор = Перечисления.ИИА_АвторСообщения.Пользователь Тогда
					...
					СтрокаСообщения.ЗадачаВыполнена = ЗадачаВыполнена;
					Диалог.Записать();
...
// Обрабатывает результат выполнения DSL (успех или ошибка)
//
// Параметры:
//  РезультатDSL - Структура - результат выполнения DSL
//  СсылкаДиалога - СправочникСсылка.ИИА_Диалоги - ссылка на диалог
//  ТекстПользователя - Строка - текст задачи пользователя (для проверки результата)
//  Настройки - Структура - настройки пользователя
//  DSLJSON - Строка - исходный DSL-сценарий (для повторных попыток при ошибке)
//  ЭтоПерваяПопытка - Булево - является ли это первой попыткой выполнения
//  НомерПопытки - Число - номер текущей попытки (для повторных попыток)
//  МаксимальноеКоличествоПопыток - Число - максимальное количество попыток исправления
//
Процедура ОбработатьРезультатDSL(РезультатDSL, СсылкаДиалога, ТекстПользователя, Настройки, DSLJSON, ЭтоПерваяПопытка, НомерПопытки = 0, МаксимальноеКоличествоПопыток = 5)
	...
	Если РезультатDSL.Успех Тогда
		...
		// Формируем текст успешного выполнения
		Если ЭтоПерваяПопытка Тогда
			ТекстСистемногоСообщения = "DSL-сценарий выполнен успешно.";
		Иначе
			ТекстСистемногоСообщения = "Исправленный DSL-сценарий выполнен успешно (попытка " + Формат(НомерПопытки, "ЧН=0") + ").";
		КонецЕсли;
		...
		// Проверяем результат выполнения задачи через ИИ (только для первой попытки)
		Если ЭтоПерваяПопытка Тогда
			ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога, ТекстПользователя, РезультатDSL, Настройки);
		КонецЕсли;
	...
	Иначе
		...
		Если НомерПопытки < МаксимальноеКоличествоПопыток Тогда
			...
			// Пытаемся исправить ошибку через ИИ
			РезультатКоррекции = ИИА_Провайдеры.ИсправитьDSLЧерезИИ(DSLJSON, РезультатDSL.Сообщение, Настройки, НомерПопытки + 1, МаксимальноеКоличествоПопыток);
			...
			Если РезультатКоррекции.Успех Тогда
				...
				ОбработатьРезультатDSL(РезультатИсправленногоDSL, СсылкаДиалога, ТекстПользователя, Настройки, РезультатКоррекции.ИсправленныйDSL, Ложь, НомерПопытки + 1, МаксимальноеКоличествоПопыток);
				
			Иначе
				// Если не удалось получить исправленный DSL, пытаемся еще раз
				Если НомерПопытки + 1 < МаксимальноеКоличествоПопыток Тогда
					ОбработатьРезультатDSL(РезультатDSL, СсылкаДиалога, ТекстПользователя, Настройки, DSLJSON, Ложь, НомерПопытки + 1, МаксимальноеКоличествоПопыток);
				Иначе
					...
					ДобавитьСообщениеВДиалог(
						...
						Перечисления.ИИА_АвторСообщения.Система,
						Перечисления.ИИА_ТипСообщения.Ошибка,
						"Не удалось получить исправленный DSL от ИИ (попытка " + Формат(НомерПопытки + 1, "ЧН=0") + "): " + РезультатКоррекции.Сообщение + Символы.ПС + "Достигнут лимит попыток исправления (" + Формат(МаксимальноеКоличествоПопыток, "ЧН=0") + ").",
						"",
						"",
						0
					);
