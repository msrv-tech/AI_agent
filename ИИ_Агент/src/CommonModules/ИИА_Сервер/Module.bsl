...
//
Функция СоздатьНовыйДиалог(Пользователь) Экспорт
...
Процедура ДобавитьИзмененныеОбъекты(СсылкаДиалога, МассивСсылок) Экспорт
	...
	Исключение
		// Игнорируем ошибки добавления объектов
	КонецПопытки;
...
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения) Экспорт
	...
	Если ОтветИИ.ТипОтвета = "DSL" И НЕ ПустаяСтрока(ОтветИИ.DSL) Тогда
		...
		РезультатDSL = ИИА_DSL.ВыполнитьDSL(ОтветИИ.DSL);
...
Функция ПолучитьДанныеСозданныхОбъектов(МассивСсылок)
	...
	Для Каждого СсылкаОбъекта Из МассивСсылок Цикл
		...
		Попытка
			...
			Если ТипСсылки = Тип("СправочникСсылка") Тогда
			...
			ИначеЕсли ТипСсылки = Тип("ДокументСсылка") Тогда
...
Функция ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога, ТекстЗадачиПользователя, РезультатDSL, Настройки = Неопределено) Экспорт
	...
	Попытка
		...
		ПромптПроверки = "Сгенерируй DSL-сценарий для проверки результата выполнения задачи." + Символы.ПС +
			"Задача пользователя: " + ТекстЗадачиПользователя + Символы.ПС +
			Символы.ПС +
			"Результат выполнения DSL-сценария:" + Символы.ПС +
			ТекстРезультатов + Символы.ПС +
			Символы.ПС +
			"КРИТИЧЕСКИ ВАЖНО: Тщательно проверь выполнение задачи!" + Символы.ПС +
			"Для проверки используй только действия, которые НЕ изменяют базу данных: GetChangedObjects, RunQuery, CheckObjectExists, ShowInfo." + Символы.ПС +
			"Не применяй CreateReference/CreateDocument/SetField/Write/SaveToStorage/LoadFromStorage – проверка должна быть чисто диагностической." + Символы.ПС +
			"Для подтверждения правильности выполнения используй RunQuery для выборки нужных объектов и реквизитов." + Символы.ПС +
			"Если задача связана с заполнением реквизита, обязательно проверь, что значение установлено у ВСЕХ нужных объектов (RunQuery с группировкой)." + Символы.ПС +
			Символы.ПС +
			"КРИТИЧЕСКИ ВАЖНО о проверке заполнения реквизитов:" + Символы.ПС +
			"Если задача была 'заполнить реквизит X = Y у всех объектов Z', то:" + Символы.ПС +
			"1. Выполни RunQuery для проверки: ВЫБРАТЬ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Объекты.Ссылка) КАК ВсегоОбъектов, " +
			"КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Объекты.Ссылка) ГДЕ Объекты.Реквизит = Y КАК Заполненных " +
			"ИЗ Справочник.Z КАК Объекты" + Символы.ПС +
			"2. Задача ВЫПОЛНЕНА только если ВсегоОбъектов = Заполненных (все объекты имеют заполненный реквизит)" + Символы.ПС +
			"3. Если ВсегоОбъектов > Заполненных - задача НЕ ВЫПОЛНЕНА, нужно продолжить заполнение" + Символы.ПС +
			"4. Если в результатах DSL-сценария есть только RunQuery (получение списка объектов) и никаких действий изменения, " +
			"то задача скорее всего проверяется, а не выполняется." + Символы.ПС +
			Символы.ПС +
			"ВАЖНО ДЛЯ ЗАДАЧ 'ВЫВЕСТИ ДАННЫЕ' (например, 'покажи', 'выведи список'):" + Символы.ПС +
			"Задача считается ВЫПОЛНЕННОЙ ТОЛЬКО если в предыдущих шагах БЫЛ ВЫПОЛНЕН RunQuery, который вернул запрошенные данные." + Символы.ПС +
			"Если был выполнен только GetMetadata или GetObjectFields, но НЕ БЫЛО RunQuery с выборкой данных - задача НЕ ВЫПОЛНЕНА! Отвечай 'НЕТ'." + Символы.ПС +
			Символы.ПС +
			"ВАЖНО ДЛЯ ЗАДАЧ НА ИЗМЕНЕНИЕ ДАННЫХ (создать, изменить, пометить, удалить и т.д.):" + Символы.ПС +
			"Задача считается ВЫПОЛНЕННОЙ ТОЛЬКО если в предыдущих шагах БЫЛИ ВЫПОЛНЕНЫ действия Write, Delete или аналогичные, изменяющие данные." + Символы.ПС +
			"Если был выполнен только RunQuery (даже если он показал правильные данные для изменения), но не было Write/Delete - задача НЕ ВЫПОЛНЕНА! Отвечай 'НЕТ'." + Символы.ПС +
			Символы.ПС +
			"Проверь:" + Символы.ПС +
			"1. Соответствует ли результат задаче пользователя" + Символы.ПС +
			"2. Правильно ли заполнены реквизиты созданных/измененных объектов (используй RunQuery для проверки реальных значений в БД)" + Символы.ПС +
			"3. Все ли необходимые поля установлены" + Символы.ПС +
			"4. Если задача была заполнить реквизит - проверь через RunQuery, что реквизит действительно заполнен у ВСЕХ нужных объектов" + Символы.ПС +
			Символы.ПС +
			"ВАЖНО: Если задача пользователя была создать объект (например, контрагента), и объект уже создан и записан в базу данных, то задача ВЫПОЛНЕНА. " +
			"Не нужно создавать объект повторно или выполнять дополнительные действия, если объект уже существует и правильно заполнен." + Символы.ПС +
			"Если задача была заполнить реквизит - задача ВЫПОЛНЕНА только если реквизит действительно заполнен у ВСЕХ нужных объектов (проверь через RunQuery)!" + Символы.ПС +
			"Если задача была вывести данные - задача ВЫПОЛНЕНА только если данные были выведены пользователю!" + Символы.ПС +
			Символы.ПС +
			"В конце сценария однозначно используй ShowInfo с сообщением 'ДА', если задача выполнена полностью, или 'НЕТ', если нет. Дополнительно укажи причину (какой RunQuery подтвердил результат).";
...
Функция СгенерироватьSummary(СсылкаДиалога) Экспорт
	...
	Попытка
		...
		ПромптSummary = "Создай краткое резюме (summary) выполненной работы на основе истории диалога." + Символы.ПС +
			Символы.ПС +
			"Задачи пользователя:" + Символы.ПС +
			ТекстЗадач + Символы.ПС +
			Символы.ПС +
			ТекстИзмененныхОбъектов + Символы.ПС;
		
		// Добавляем результаты выполнения DSL, если они есть
		Если НЕ ПустаяСтрока(ТекстРезультатовDSL) Тогда
			ПромптSummary = ПромптSummary + Символы.ПС +
				"Результаты выполнения DSL-сценариев:" + Символы.ПС +
				ТекстРезультатовDSL + Символы.ПС;
		КонецЕсли;
		
		ПромптSummary = ПромптSummary + Символы.ПС +
			"ВАЖНО: Используй ТОЛЬКО реальные данные из результатов выполнения DSL-сценариев выше." + Символы.ПС +
			"НЕ придумывай и НЕ предполагай данные, которых нет в результатах!" + Символы.ПС +
			"Если в результатах нет конкретных данных (например, списка реквизитов), укажи это в резюме." + Символы.ПС +
			Символы.ПС +
			"Резюме должно содержать:" + Символы.ПС +
			"1. Краткое описание того, что было сделано" + Символы.ПС +
			"2. Список созданных или измененных объектов (если есть)" + Символы.ПС +
			"3. Ответы на вопросы пользователя (если были вопросы) - используй ТОЛЬКО реальные данные из результатов DSL" + Символы.ПС +
			"4. Любые важные детали или результаты работы" + Символы.ПС +
			Символы.ПС +
			"Резюме должно быть написано на русском языке, структурированно и понятно для пользователя." + Символы.ПС +
			"Используй историю диалога для понимания контекста и всех выполненных действий.";
