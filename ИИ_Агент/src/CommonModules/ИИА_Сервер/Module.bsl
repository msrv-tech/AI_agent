...
Функция ОтправитьСообщениеСервера(СсылкаДиалога, ТипСообщения, ТекстПользователя) Экспорт
	...
	ИначеЕсли ОтветИИ.ТипОтвета = "DSL" Тогда
		...
		Иначе
			// Если DSL не выполняется, добавляем информацию для отладки
			Если ОтветИИ.Свойство("ВыполнитьDSL") Тогда
				ТекстОтвета = ТекстОтвета + Символы.ПС + "DSL-сценарий не выполнен (ВыполнитьDSL = " + Строка(ОтветИИ.ВыполнитьDSL) + ")";
			Иначе
				ТекстОтвета = ТекстОтвета + Символы.ПС + "DSL-сценарий не выполнен (флаг ВыполнитьDSL отсутствует)";
			КонецЕсли;
