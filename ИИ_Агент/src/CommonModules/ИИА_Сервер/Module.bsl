...
Функция ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога, ТекстЗадачиПользователя, РезультатDSL, Настройки = Неопределено) Экспорт
	...
	Попытка
		...
		ПромптПроверки = "Сгенерируй DSL-сценарий для проверки результата выполнения задачи." + Символы.ПС +
			...
			"2. Для каждого объекта используй FindReferenceByGUID (с УИД из GetChangedObjects) для получения ссылки" + Символы.ПС +
			"3. RunQuery для проверки значений реквизитов объектов в базе данных" + Символы.ПС +
			"Проверь:" + Символы.ПС +
			...
			"4. Если задача была заполнить реквизит - проверь, что реквизит действительно заполнен у всех нужных объектов" + Символы.ПС +
			Символы.ПС +
