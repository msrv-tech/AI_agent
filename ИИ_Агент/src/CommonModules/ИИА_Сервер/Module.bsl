...
Функция Gitsell_ОпросТокена(DeviceCode) Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если Ответ.КодСостояния = 200 Тогда
				Если Данные.Свойство("ai_proxy_token") Тогда
					Результат.Статус = "success";
					Результат.Токен = Данные.ai_proxy_token;
				Иначе
					Результат.Ошибка = "В ответе нет ai_proxy_token";
				КонецЕсли;
			Иначе
		...
		Исключение
			Результат.Ошибка = "Ошибка парсинга ответа: " + ОписаниеОшибки() + " | " + ТекстОтвета;
		КонецПопытки;
...
//  Токен - Строка
//
Процедура Gitsell_СохранитьAiProxyToken(Токен) Экспорт
	
	Пользователь = "Админ"; // Используем фиксированного пользователя как "системного" для общего токена
...
Функция Gitsell_ПроверитьModels() Экспорт
	...
	Попытка
		...
		Настройки = ПолучитьНастройкиПользователя("Админ");
