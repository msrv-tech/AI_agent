...
Функция ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога, ТекстЗадачиПользователя, РезультатDSL, Настройки = Неопределено) Экспорт
	...
	Попытка
		...
		ПромптПроверки = "Сгенерируй DSL-сценарий для проверки результата выполнения задачи." + Символы.ПС +
			...
			"Для проверки используй следующие действия:" + Символы.ПС +
			"1. GetChangedObjects - чтобы получить все объекты из табличной части 'ИзмененныеОбъекты' диалога" + Символы.ПС +
			"2. RunQuery для проверки значений реквизитов объектов в базе данных (ОБЯЗАТЕЛЬНО!)" + Символы.ПС +
			"3. Если задача была заполнить реквизит - используй RunQuery для проверки, что реквизит действительно заполнен у ВСЕХ нужных объектов" + Символы.ПС +
			Символы.ПС +
			...
			"4. Если в результатах DSL-сценария есть только RunQuery (получение списка объектов) и SaveToStorage (сохранение в хранилище), " +
			"но НЕТ SetField и Write - это означает, что задача только ПОДГОТОВЛЕНА, но НЕ ВЫПОЛНЕНА!" + Символы.ПС +
			Символы.ПС +
			"Проверь:" + Символы.ПС +
			...
			"5. Если в DSL-сценарии были только действия подготовки (GetObjectFields, RunQuery, SaveToStorage), но не было действий изменения (SetField, Write), " +
			"то задача НЕ ВЫПОЛНЕНА - нужно продолжить выполнение!" + Символы.ПС +
			Символы.ПС +
			...
			Символы.ПС +
			"В конце сценария используй ShowInfo с сообщением 'ДА', если задача выполнена полностью, или 'НЕТ', если нет.";
