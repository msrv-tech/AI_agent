...
Функция ПолучитьНастройкиПользователя(Пользователь) Экспорт
	...
	Запрос.УстановитьПараметр("Пользователь", "Админ");
...
Функция ВызватьГигачат(ТипСообщения, ТекстПользователя, История, Настройки)
	...
	Если ПустаяСтрока(URLИзНастроек) Тогда
		URLИзНастроек = "https://gigachat.devices.sberbank.ru";
	КонецЕсли;
	...
	// Разделяем URL на базовый домен и путь
	// Ищем первый слеш после https:// (позиция 9 - после "https://")
	ПозицияСлеша = СтрНайти(URLИзНастроек, "/", НаправлениеПоиска.СНачала, 9);
	Если ПозицияСлеша > 0 Тогда
		БазовыйURL = Лев(URLИзНастроек, ПозицияСлеша - 1);
		БазовыйПуть = Сред(URLИзНастроек, ПозицияСлеша);
		// Если в пути уже есть /api/v1, добавляем только /chat/completions
		Если СтрНайти(БазовыйПуть, "/api/v1") > 0 Тогда
			URLПуть = БазовыйПуть + "/chat/completions";
		Иначе
			URLПуть = БазовыйПуть + "/api/v1/chat/completions";
		КонецЕсли;
	Иначе
		БазовыйURL = URLИзНастроек;
	...
	Исключение
		...
		Результат.Текст = "Ошибка при вызове API Гигачат: " + ТекстОшибки + 
			Символы.ПС + "БазовыйURL: " + БазовыйURL + 
