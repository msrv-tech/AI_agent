...
Функция ПолучитьЛогДиалога(СсылкаДиалога) Экспорт
	...
	// Упрощенное получение значения реквизита
	// Платформа сама оптимизирует это в запрос
	Попытка
		Возврат Строка(СсылкаДиалога.Лог);
	Исключение
		Возврат "Ошибка чтения лога (возможно объект не найден или недоступен): " + ОписаниеОшибки();
	КонецПопытки;
