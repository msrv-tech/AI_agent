...
Функция ВыполнитьDSLСценарий(DSLJSON, СсылкаДиалога = Неопределено) Экспорт
	...
	Возврат ИИА_DSL.ВыполнитьDSL(DSLJSON, СсылкаДиалога);
...
Функция ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога, ТекстЗадачиПользователя, РезультатDSL, Настройки = Неопределено) Экспорт
	...
	Попытка
		...
		ПромптПроверки = "Сгенерируй DSL-сценарий для проверки результата выполнения задачи." + Символы.ПС +
			...
			Символы.ПС +
			"Для проверки используй действие GetChangedObjects, чтобы получить все объекты из табличной части 'ИзмененныеОбъекты' диалога." + Символы.ПС +
			"Затем проверь каждый объект, используя FindReferenceByName и SetField для чтения значений реквизитов." + Символы.ПС +
			"Проверь:" + Символы.ПС +
			...
			Символы.ПС +
			"В конце сценария используй ShowInfo с сообщением 'ДА', если задача выполнена, или 'НЕТ', если нет.";
