...
Функция ПолучитьНастройкиПользователя(Пользователь) Экспорт
	...
	Результат.Вставить("ЛимитТокеновНаДиалог", Выборка.ЛимитТокеновНаДиалог);
...
Функция ИсправитьDSLЧерезИИ(СсылкаДиалога, ТекстОшибки, ИсходныйDSL) Экспорт
	...
	Если СтрНайти(ТекстОшибки, "RunQuery") > 0 
		...
		Иначе
			// Иначе подмешиваем полную справку по синтаксису
...
Функция ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога) Экспорт
	...
	Если СтрНачинаетсяС(ТекстОтветаВРег, "ДА") 
		...
		Если Диалог.ТипДиалога = Перечисления.ИИА_ТипДиалога.Запрос1С Тогда
			...
			Если НЕ ЕстьТабличныйДокумент Тогда
				Результат.ЗадачаВыполнена = Ложь;
			...
			Иначе
				Результат.ЗадачаВыполнена = Истина;
	...
	УстановитьРезультатПроверкиВХранилище(СсылкаДиалога, Результат.ЗадачаВыполнена, Результат.Причина);
...
Функция СгенерироватьПродолжениеВыполнения(СсылкаДиалога) Экспорт
	...
	Если Диалог.План.Количество() > 0 Тогда
		...
		// Сначала обновляем план по результатам
		ОбновитьПланПоРезультатам(СсылкаДиалога);
		
		// Проверяем, завершен ли план
		Если ПланЗавершен(СсылкаДиалога) Тогда
			// План завершен, вызываем обычную проверку на всякий случай
			// или просто возвращаем Ложь
			Возврат Ложь;
		КонецЕсли;
		
		// Получаем следующую задачу из плана
		СледующаяЗадача = ПолучитьСледующийПунктПлана(СсылкаДиалога);
		Если ПустаяСтрока(СледующаяЗадача) Тогда
			Возврат Ложь;
		КонецЕсли;
		
		// Формируем промпт на основе плана
		МассивПлана = Новый Массив;
		Для Каждого СтрокаПлана Из Диалог.План Цикл
		...
		ПромптПродолжения = ИИА_Промты.ПолучитьПромптВыполненияПунктаПлана(ПланJSON, СледующаяЗадача);
	...
	Иначе
		...
		Если РезультатПроверки = Неопределено ИЛИ РезультатПроверки.ПроверкаВыполнена = Ложь ИЛИ РезультатПроверки.ЗадачаВыполнена = Истина Тогда
			Возврат Ложь;
		КонецЕсли;
