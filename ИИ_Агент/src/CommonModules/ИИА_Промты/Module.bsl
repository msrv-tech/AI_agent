...
Функция СформироватьПромптПроверкиЗадачи(ТекстЗадачиПользователя, ТекстРезультатов) Экспорт
	...
	"1. RunQuery - для проверки данных в базе (ОБЯЗАТЕЛЬНО, если задача требовала ВЫВОДА данных или ИЗМЕНЕНИЯ данных)" + Символы.ПС +
	...
	"" + Символы.ПС +
	"Если задача требовала ВЫВЕСТИ данные, ты ОБЯЗАН выполнить RunQuery, чтобы убедиться, что данные выбраны." + Символы.ПС +
	"Если ты выполнил только GetMetadata или GetObjectFields, а задача требует данных - это НЕ выполнение." + Символы.ПС +
	"Если задача требовала ИЗМЕНИТЬ данные, ты ОБЯЗАН выполнить RunQuery или CheckObjectExists, чтобы убедиться, что изменения применены." + Символы.ПС +
	"" + Символы.ПС +
	...
	"Пример: ShowInfo с message: 'ДА. Все контрагенты помечены на удаление.' или 'НЕТ. Не удалось найти созданный документ.'";
