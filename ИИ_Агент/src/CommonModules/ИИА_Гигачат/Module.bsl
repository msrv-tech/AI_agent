...
Функция ВызватьГигачат(ТипСообщения, ТекстПользователя, История, Настройки) Экспорт
	...
	МетаданныеКонтекст = ИИА_Провайдеры.ПолучитьМетаданныеДляИИ();
	
	// Формируем промпт с учетом типа сообщения и метаданных
	Промпт = ИИА_Провайдеры.СформироватьПромптДляИИ(ТипСообщения, ТекстПользователя, История, МетаданныеКонтекст);
