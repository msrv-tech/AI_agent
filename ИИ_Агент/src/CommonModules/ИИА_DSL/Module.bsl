...
Функция РазрешенныеДействия()
	...
	Действия.Добавить("SaveToStorage");
	Действия.Добавить("LoadFromStorage");
	Действия.Добавить("ClearStorage");
	Действия.Добавить("ListStorageKeys");
...
Функция ВалидироватьШаг(Шаг, НомерШага)
	...
	ИначеЕсли Действие = "ClearStorage" Тогда
		
		// ClearStorage может работать без параметров (очищает все) или с параметром key (очищает конкретный ключ)
		// Валидация успешна, если действие указано
		
	ИначеЕсли Действие = "ListStorageKeys" Тогда
		
		// ListStorageKeys не требует параметров
...
Функция ВыполнитьШаг(Шаг, КонтекстВыполнения)
	...
	Попытка
		...
		ИначеЕсли Действие = "FindReferenceByGUID" Тогда
			...
			Результат = ВыполнитьFindReferenceByGUID(Шаг, КонтекстВыполнения);
			
		ИначеЕсли Действие = "FindReferenceByURL" Тогда
			
			Результат = ВыполнитьFindReferenceByURL(Шаг, КонтекстВыполнения);
		...
		ИначеЕсли Действие = "GetMetadata" Тогда
			...
			Результат = ВыполнитьGetMetadata(Шаг, КонтекстВыполнения);
			
		ИначеЕсли Действие = "GetObjectFields" Тогда
			
			Результат = ВыполнитьGetObjectFields(Шаг, КонтекстВыполнения);
			
		ИначеЕсли Действие = "CheckObjectExists" Тогда
			
			Результат = ВыполнитьCheckObjectExists(Шаг, КонтекстВыполнения);
			
		ИначеЕсли Действие = "SaveToStorage" Тогда
			
			Результат = ВыполнитьSaveToStorage(Шаг, КонтекстВыполнения);
			
		ИначеЕсли Действие = "LoadFromStorage" Тогда
			
			Результат = ВыполнитьLoadFromStorage(Шаг, КонтекстВыполнения);
			
		ИначеЕсли Действие = "ClearStorage" Тогда
			
			Результат = ВыполнитьClearStorage(Шаг, КонтекстВыполнения);
			
		ИначеЕсли Действие = "ListStorageKeys" Тогда
			
			Результат = ВыполнитьListStorageKeys(Шаг, КонтекстВыполнения);
...
Функция ВыполнитьFindReferenceByGUID(Шаг, КонтекстВыполнения)
	...
	Если ТипЗнч(НайденнаяСсылка) = Тип("СправочникСсылка") Тогда
		ТипОбъекта = "Справочник";
		ИмяОбъекта = НайденнаяСсылка.Метаданные().Имя;
	ИначеЕсли ТипЗнч(НайденнаяСсылка) = Тип("ДокументСсылка") Тогда
		ТипОбъекта = "Документ";
		ИмяОбъекта = НайденнаяСсылка.Метаданные().Имя;
	Иначе
...
// Выполняет действие ClearStorage
//
// Параметры:
//  Шаг - Структура - структура шага
//  КонтекстВыполнения - Структура - контекст выполнения (входной/выходной)
...
//  Структура - структура результата
//
Функция ВыполнитьClearStorage(Шаг, КонтекстВыполнения)
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Диалог.ХранилищеЗначения) Тогда
			Результат.Успех = Истина;
			Результат.Сообщение = "Хранилище уже пусто";
			Результат.Данные = Истина;
			Возврат Результат;
		КонецЕсли;
		...
		// Если указан ключ, удаляем только его, иначе очищаем все
		// ХранилищеЗначения не имеет метода Удалить - всегда создаем новое хранилище
		Если Шаг.Свойство("key") И ЗначениеЗаполнено(Шаг.key) Тогда
			
			Ключ = Шаг.key;
			
			// Получаем текущую структуру из хранилища
			ТекущаяСтруктура = Хранилище.Получить();
			Если ТипЗнч(ТекущаяСтруктура) = Тип("Структура") И ТекущаяСтруктура.Свойство(Ключ) Тогда
				// Создаем новую структуру без удаляемого ключа
				НоваяСтруктура = Новый Структура;
				Для Каждого КлючЗначение Из ТекущаяСтруктура Цикл
					Если КлючЗначение.Ключ <> Ключ Тогда
						НоваяСтруктура.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
					КонецЕсли;
				КонецЦикла;
				// Создаем новое хранилище с обновленной структурой
				Диалог.ХранилищеЗначения = Новый ХранилищеЗначения(НоваяСтруктура);
				Результат.Успех = Истина;
				Результат.Сообщение = "Ключ '" + Ключ + "' удален из хранилища";
			Иначе
				Результат.Успех = Истина;
				Результат.Сообщение = "Ключ '" + Ключ + "' не найден в хранилище";
			КонецЕсли;
		...
		Иначе
			...
			// ХранилищеЗначения требует обязательный параметр - создаем с пустой структурой
			ПустаяСтруктура = Новый Структура;
			Диалог.ХранилищеЗначения = Новый ХранилищеЗначения(ПустаяСтруктура);
			Результат.Успех = Истина;
			Результат.Сообщение = "Хранилище полностью очищено";
	...
	Исключение
		Результат.Сообщение = "Ошибка при очистке хранилища: " + ОписаниеОшибки();
	КонецПопытки;
...
// Выполняет действие ListStorageKeys
//
// Параметры:
//  Шаг - Структура - структура шага
//  КонтекстВыполнения - Структура - контекст выполнения (входной/выходной)
...
//  Структура - структура результата
//
Функция ВыполнитьListStorageKeys(Шаг, КонтекстВыполнения)
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Диалог.ХранилищеЗначения) Тогда
			Результат.Успех = Истина;
			Результат.Сообщение = "Хранилище пусто";
		...
		Результат.Успех = Истина;
		Результат.Сообщение = "Получен список ключей из хранилища (всего: " + МассивКлючей.Количество() + ")";
		Результат.Данные = МассивКлючей;
		
	Исключение
		Результат.Сообщение = "Ошибка при получении списка ключей: " + ОписаниеОшибки();
	КонецПопытки;
