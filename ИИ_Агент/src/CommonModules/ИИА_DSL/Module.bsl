...
Функция ВалидироватьШаг(Шаг, НомерШага)
	...
	ИначеЕсли Действие = "SelectObject" Тогда
		...
		Если НЕ Шаг.Свойство("object") Тогда
			Результат.Успех = Ложь;
			Результат.Сообщение = "Шаг " + Формат(НомерШага, "ЧН=0") + ": SelectObject требует поле object";
			Возврат Результат;
		КонецЕсли;
...
Функция ВыполнитьSelectObject(Шаг, КонтекстВыполнения)
	...
	Попытка
		...
		// Определяем тип для логики Write
		...
		Если Части.Количество() >= 1 Тогда
			...
			Если КонтекстВыполнения.Свойство(ИмяПеременной) Тогда
				...
				Для Индекс = 1 По Части.Количество() - 1 Цикл
					...
					Если ТипЗнч(ТекущееЗначение) = Тип("Структура") И ТекущееЗначение.Свойство(ИмяСвойства) Тогда
						ТекущееЗначение = ТекущееЗначение[ИмяСвойства];
					ИначеЕсли (ТипЗнч(ТекущееЗначение) = Тип("СправочникСсылка") Или ТипЗнч(ТекущееЗначение) = Тип("ДокументСсылка")) Тогда
						Попытка
							ТекущееЗначение = ТекущееЗначение[ИмяСвойства];
						Исключение
