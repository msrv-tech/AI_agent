		...
		Для Каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
			...
			Для Каждого Колонка Из ТаблицаРезультатов.Колонки Цикл
				Попытка
					...
					Иначе
						// Проверяем, является ли значение ссылкой (используем строковое представление типа)
						СтрокаТипа = Строка(ТипЗнч(ЗначениеКолонки));
						Если СтрНайти(СтрокаТипа, "Ссылка") > 0 Тогда
							// Если это ссылка, сохраняем её строковое представление и извлекаем УИД
							СсылкаОбъекта = ЗначениеКолонки;
							СтруктураСтроки.Вставить(Колонка.Имя, Строка(СсылкаОбъекта));
							Попытка
								УИДИзСсылки = Строка(СсылкаОбъекта.УникальныйИдентификатор());
								НайденаСсылка = Истина;
							Исключение
								// Если не удалось получить УИД, продолжаем без него
							КонецПопытки;
						Иначе
...
Функция ОбработатьЗначениеПараметра(Значение, КонтекстВыполнения)
	...
	Если СтрНайти(Значение, "#(") = 1 И Прав(Значение, 1) = ")" Тогда
		...
		// Если это прямая ссылка на переменную контекста (например, #(Хранилище_Ключ))
		Если КонтекстВыполнения.Свойство(ПутьКЗначению) Тогда
			Возврат КонтекстВыполнения[ПутьКЗначению];
		КонецЕсли;
		
		// Разбираем путь (например, ШАГ1.Данные)
		Части = СтрРазделить(ПутьКЗначению, ".");
		
		Если Части.Количество() >= 1 Тогда
			
			ИмяШага = Части[0]; // Например, ШАГ1
			
			Если КонтекстВыполнения.Свойство(ИмяШага) Тогда
				
				Объект = КонтекстВыполнения[ИмяШага];
				
				// Если есть вторая часть, пытаемся получить свойство объекта
				Если Части.Количество() >= 2 Тогда
					
					ИмяСвойства = Части[1];
					
					If ТипЗнч(Объект) = Тип("Структура") И Объект.Свойство(ИмяСвойства) Тогда
						Возврат Объект[ИмяСвойства];
					ИначеЕсли (ТипЗнч(Объект) = Тип("СправочникСсылка") Или ТипЗнч(Объект) = Тип("ДокументСсылка")) Тогда
						Попытка
							Возврат Объект[ИмяСвойства];
						Исключение
							// Если не удалось получить свойство, возвращаем исходную строку
							Возврат Значение;
						КонецПопытки;
					Иначе
						Возврат Значение;
				...
				Иначе
					// Если только имя шага, возвращаем весь результат шага
					Возврат Объект;
				КонецЕсли;
