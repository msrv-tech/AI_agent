...
Процедура ОпросТокена()
	...
	Если Результат.Статус = "success" Тогда
		...
		// Сохраняем токен
		Gitsell_СохранитьТокенНаСервере(Результат.Токен);
		...
		Если Проверка.Успех Тогда
			...
			// Закрываем форму через небольшую паузу или сразу
			Сообщить("Авторизация прошла успешно. Можно закрыть окно.");
		Иначе
...
&НаСервере
Процедура Gitsell_СохранитьТокенНаСервере(Токен)
	ИИА_Сервер.Gitsell_СохранитьAiProxyToken(Токен);
КонецПроцедуры
