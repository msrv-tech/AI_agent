...
Процедура КомандаОтправить(Команда)
	...
	Попытка
		...
		// Добавляем промпт в лог, если он есть (не затирая историю)
		ДобавитьПромптВЛог(Результат);
		
		// Обновляем список сообщений
		ОбновитьСообщения();
		
		// Обрабатываем результат DSL, если есть (исправление ошибок и итерации до выполнения задачи на клиенте)
		Если Результат <> Неопределено 
			И Результат.Свойство("РезультатDSL") 
			И Результат.РезультатDSL <> Неопределено 
			И Результат.Свойство("DSLJSON") Тогда
			
			// Запускаем обработку результата DSL с попытками исправления и итерациями до выполнения задачи на клиенте
			...
			// Обновляем список сообщений после обработки
			ОбновитьСообщения();
		КонецЕсли;
...
Процедура ДобавитьПромптВЛог(Результат)
	...
	Если Результат <> Неопределено И Результат.Свойство("Промпт") И НЕ ПустаяСтрока(Результат.Промпт) Тогда
		...
		Иначе
			Лог = Лог + Символы.ПС + Символы.ПС + "=== Следующий промпт ===" + Символы.ПС + Символы.ПС + Результат.Промпт;
		КонецЕсли;
	КонецЕсли;
