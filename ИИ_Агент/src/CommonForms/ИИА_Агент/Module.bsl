...
Процедура СпланироватьСледующиеШаги()
	...
	Попытка
		...
		Если Настройки.Свойство("ЛимитТокеновНаДиалог") И Настройки.ЛимитТокеновНаДиалог > 0 Тогда
			МаксимальноеКоличествоТокенов = Настройки.ЛимитТокеновНаДиалог;
		КонецЕсли;
	Исключение
	КонецПопытки;
	...
	// 3. Если последний шаг был DSL и он выполнен, обновляем план
	ПоследнееDSLСообщение = ИИА_ВызовСервера.ПолучитьПоследнееDSLСообщение(ТекущийДиалог);
	Если ПоследнееDSLСообщение.Найдено И ПоследнееDSLСообщение.СтатусDSL = ПредопределенноеЗначение("Перечисление.ИИА_СтатусDSL.ОбработанУспешно") Тогда
		СтрокаСтатуса = "Обновление статуса плана...";
		ИИА_ВызовСервера.ОбновитьПланПоРезультатам(ТекущийДиалог);
		ОбновитьПланНаФорме();
	КонецЕсли;
	...
	Если ИИА_ВызовСервера.ПланЗавершен(ТекущийДиалог) Тогда
		...
		ИИА_ВызовСервера.УстановитьРезультатПроверки(ТекущийДиалог, ЗадачаВыполнена, РезультатПроверки.Причина);
		СгенерироватьИДобавитьSummary(ТекущийДиалог, ЗадачаВыполнена, РезультатПроверки.Причина);
		
		Если ЗадачаВыполнена Тогда
			СтрокаСтатуса = "Задача выполнена успешно";
			ОстановитьОркестратор();
		Иначе
			СтрокаСтатуса = "Требуется уточнение плана";
			// Здесь можно либо допланировать, либо просто остановить для пользователя
			// Для начала просто остановим
			ОстановитьОркестратор();
		КонецЕсли;
	...
	СтрокаСтатуса = "Генерация следующего шага...";
