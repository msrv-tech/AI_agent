...
Процедура СпланироватьСледующиеШаги()
	...
	// Лимит токенов на одну обработку задачи (получаем из настроек пользователя)
	МаксимальноеКоличествоТокенов = 10000; // Значение по умолчанию
	...
	Если РезультатDSL.Успех Тогда
			...
			Если ЗадачаВыполнена Тогда
				...
				Возврат;
			Иначе
				СтрокаСтатуса = "Шаг выполнен. Продолжаю выполнение...";
				ДобавитьВЛог("Система", "Проверка показала, что задача еще не выполнена. Генерирую следующий шаг...", Новый Структура("Причина", ПричинаПроверки));
				
			ИИА_ВызовСервера.ДобавитьСистемноеСообщениеНаСервере(
				ТекущийДиалог,
				"Шаг выполнен. Задача требует продолжения. Генерирую следующий шаг...",
				ПредопределенноеЗначение("Перечисление.ИИА_ТипСообщения.Текст")
			);
			...
			// Сбрасываем результат проверки в хранилище, чтобы на следующем шаге он не блокировал выполнение
...
Процедура СоздатьИОткрытьТабличныйДокумент(ДанныеЗапроса)
	...
	Попытка
		...
		// Сообщаем серверу, что таблица была показана (это важный триггер для завершения задачи)
