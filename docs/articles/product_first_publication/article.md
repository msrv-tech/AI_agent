...
Чтобы не обсуждать “ну вроде работает”, у нас есть тестовый контур на Python, который гоняет сценарии через HTTP-bridge и собирает отчёт с метриками.

- **Запуск**: `run_tests.py` (HTTP-bridge прогон, 1С “в фоне”, без ручного открытия клиента).
- **Режимы**:
