...
Функция ПолучитьСправкуФункцииДаты() Экспорт
	...
	"   Пример: ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(2025,1,1,0,0,0), ДЕНЬ, -7) — дата через ДАТАВРЕМЯ (вызов текущей даты в запросе недопустим)" + Символы.ПС +
	...
	"12. Вызов текущей даты — В ЗАПРОСАХ НЕДОПУСТИМ" + Символы.ПС +
	"    В языке запросов 1С функции получения текущей даты НЕТ. Для дат используй ДАТАВРЕМЯ(год,месяц,день,час,минута,сек) прямо в запросе." + Символы.ПС + Символы.ПС +
...
Функция ПолучитьСправкуПараметры() Экспорт
	...
	"ВАЖНО: В языке запросов 1С функция получения текущей даты НЕДОПУСТИМА. Для дат ВСЕГДА используй ДАТАВРЕМЯ(год,месяц,день,час,минута,сек) прямо в запросе. НЕ передавай даты через parameters. parameters — для ссылок, текста и др." + Символы.ПС + Символы.ПС +
...
Функция ПолучитьСправкуПоФункции(ИмяФункции) Экспорт
	...
	ИначеЕсли ИмяФункцииВерхнийРегистр = "ТЕКУЩАЯДАТА" Тогда
		Возврат "Функция получения текущей даты в языке запросов 1С НЕДОПУСТИМА. Для дат используй ДАТАВРЕМЯ(год,месяц,день,час,минута,сек) прямо в запросе.";
