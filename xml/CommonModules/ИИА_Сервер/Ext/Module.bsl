...
Функция СгенерироватьПродолжениеВыполнения(СсылкаДиалога) Экспорт
	...
	Если Диалог.План.Количество() > 0 Тогда
		...
		// 2. ФАЗА 1: ОБНОВЛЕНИЕ ПЛАНА
	ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[Система] Фаза 1: Обновление статусов плана...");
	ПромптОбновления = ИИА_Промты.ПолучитьПромптОбновленияПлана(ПланJSON, ТекстРезультатов);
	ОтветПлана = ВызватьИИССистемнымСообщением(СсылкаДиалога, "Чат", ПромптОбновления, "", "plan_json_array");
	
	ТекстПлана = ИИА_DSL.НормализоватьJSONТекст(ОтветПлана.Текст);
	Если НЕ ПустаяСтрока(ТекстПлана) Тогда
		Попытка
			ЧтениеJSON = Новый ЧтениеJSON;
			ЧтениеJSON.УстановитьСтроку(ТекстПлана);
			НовыйМассивПлана = ПрочитатьJSON(ЧтениеJSON);
			ЧтениеJSON.Закрыть();
			
			Если ТипЗнч(НовыйМассивПлана) = Тип("Массив") Тогда
				// Обновляем план в БД
				Диалог = СсылкаДиалога.ПолучитьОбъект();
				Диалог.План.Очистить();
				Для Каждого Пункт Из НовыйМассивПлана Цикл
					НоваяСтрока = Диалог.План.Добавить();
					НоваяСтрока.Задача = ?(Пункт.Свойство("Задача"), Пункт.Задача, "");
					НоваяСтрока.Выполнена = ?(Пункт.Свойство("Выполнена"), Пункт.Выполнена, Ложь);
					НоваяСтрока.Результат = ?(Пункт.Свойство("Результат"), Пункт.Результат, "");
				КонецЦикла;
				Диалог.Записать();
				ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[Система] План успешно обновлен.");
			КонецЕсли;
		Исключение
			ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[Система] Ошибка парсинга плана: " + ОписаниеОшибки());
		КонецПопытки;
	КонецЕсли;
	...
	Если НЕ ЕстьНевыполненные Тогда
		ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[Система] Все задачи выполнены. Переход к резюме.");
		Возврат Ложь;
	КонецЕсли;
	
	// 4. ФАЗА 2: ГЕНЕРАЦИЯ СЛЕДУЮЩЕГО ШАГА
	ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[Система] Фаза 2: Генерация следующего шага DSL...");
	
	// Пересобираем ПланJSON после обновления
	МассивПлана.Очистить();
	Для Каждого СтрокаПлана Из Диалог.План Цикл
		МассивПлана.Добавить(Новый Структура("Задача, Выполнена, Результат", СтрокаПлана.Задача, СтрокаПлана.Выполнена, СтрокаПлана.Результат));
	КонецЦикла;
	ЗаписьJSON = Новый ЗаписьJSON;
	...
	ПромптШага = ИИА_Промты.ПолучитьПромптГенерацииСледующегоШага(ПланJSON, ТекстРезультатов);
	ОтветИИ = ВызватьИИССистемнымСообщением(СсылкаДиалога, "Запрос", ПромптШага, "", "dsl");
	
	// 5. ВЫПОЛНЕНИЕ DSL (если получен)
	Если ОтветИИ.ТипОтвета = "DSL" И НЕ ПустаяСтрока(ОтветИИ.DSL) Тогда
		ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[Система] Автовыполнение следующего шага...");
		
		// ВАЖНО: Выполняем DSL, иначе цикл оркестратора будет бесконечно генерировать один и тот же шаг,
		// так как факты выполнения не появятся в истории.
		РезультатАвтоDSL = ВыполнитьDSLСценарийССообщениями(СсылкаДиалога, ОтветИИ.DSL);
