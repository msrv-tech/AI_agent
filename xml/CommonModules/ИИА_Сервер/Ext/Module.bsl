...
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения, СистемныйПромпт = "") Экспорт
	...
	Если СтрНайти(ТекстСистемногоСообщения, "ИСКЛЮЧИТЕЛЬНО валидным JSON массивом строк") > 0 Тогда
		Настройки.ExpectedResponseFormat = "plan_json_array";
	ИначеЕсли СтрНайти(ТекстСистемногоСообщения, """updated_plan""") > 0 И СтрНайти(ТекстСистемногоСообщения, """next_step_dsl""") > 0 Тогда
