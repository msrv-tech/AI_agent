...
Процедура ДобавитьСообщениеВДиалог(СсылкаДиалога, Автор, ТипСообщения, Текст, ТекстКода = "", Статус = "", UsageTokens = 0, СтатусDSL = Неопределено) Экспорт
	...
	// ДУБЛИРУЕМ В ЛОГ (компактно для DSL и dsl_system_result)
	ЗаписьЛога = "[" + Формат(НоваяСтрока.Время, "ДФ=yyyy-MM-dd HH:mm:ss") + "] [" + Строка(Автор) + "]";
...
//
Функция ФорматироватьКомпактноДляЛога(Текст)
	...
	Если СтрНайти(ТекстВРег, "DSL_SYSTEM_RESULT") > 0 Тогда
		...
		Если ПозMessage > 0 Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Возврат "DSL Result: " + Статус + " | " + Действие + ": " + Лев(Сообщение, 80);
	КонецЕсли;
...
Процедура ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, ЗаписьЛога, Роль = "", CallId = "") Экспорт
	...
	МеткаВремени = Формат(ТекущаяДата(), "ДФ=yyyy-MM-dd HH:mm:ss");
