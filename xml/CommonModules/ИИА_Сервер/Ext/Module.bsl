...
Функция ПолучитьОбщееКоличествоТокенов(СсылкаДиалога) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(СУММА(Сообщения.UsageTokens), 0) КАК Токены
	|ИЗ
...
Функция ПоляОбъектаЕстьВКэшеМетаданных(СсылкаДиалога, ТипОбъекта, ИмяОбъекта) Экспорт
	Запись = НайтиЗаписьКэшаМетаданныхДиалога(СсылкаДиалога, ТипОбъекта, ИмяОбъекта);
	Возврат Запись <> Неопределено;
КонецФункции

Функция НайтиЗаписьКэшаМетаданныхДиалога(СсылкаДиалога, ТипОбъекта, ИмяОбъекта) Экспорт
	Если НЕ ЗначениеЗаполнено(СсылкаДиалога) Тогда
		Возврат Неопределено;
	КонецЕсли;
	Ключ = ВРег(СтрЗаменить(СокрЛП(Строка(ТипОбъекта)) + "_" + СокрЛП(Строка(ИмяОбъекта)), ".", "_"));
	Если ПустаяСтрока(Ключ) Или Ключ = "_" Тогда
		Возврат Неопределено;
	КонецЕсли;
	СтруктураДанных = ПолучитьДанныеДиалогаИзРегистра(СсылкаДиалога);
	Если СтруктураДанных = Неопределено Или ТипЗнч(СтруктураДанных) <> Тип("Структура") Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если НЕ СтруктураДанных.Свойство("DSL_КэшМетаданных") Или ТипЗнч(СтруктураДанных.DSL_КэшМетаданных) <> Тип("Структура") Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если НЕ СтруктураДанных.DSL_КэшМетаданных.Свойство(Ключ) Тогда
		Возврат Неопределено;
	КонецЕсли;
	Возврат СтруктураДанных.DSL_КэшМетаданных[Ключ];
КонецФункции

Процедура СохранитьЗаписьКэшаМетаданныхДиалога(СсылкаДиалога, ТипОбъекта, ИмяОбъекта, Поля, Пути) Экспорт
	Если НЕ ЗначениеЗаполнено(СсылкаДиалога) Тогда
		Возврат;
	КонецЕсли;
	Ключ = ВРег(СтрЗаменить(СокрЛП(Строка(ТипОбъекта)) + "_" + СокрЛП(Строка(ИмяОбъекта)), ".", "_"));
	...
	Если СтруктураДанных.Свойство("DSL_КэшМетаданных") И ТипЗнч(СтруктураДанных.DSL_КэшМетаданных) = Тип("Структура") Тогда
		Кэш = СтруктураДанных.DSL_КэшМетаданных;
	КонецЕсли;
	Запись = Новый Структура("ТипОбъекта,ИмяОбъекта,Поля,Пути", Строка(ТипОбъекта), Строка(ИмяОбъекта), Поля, Пути);
	Кэш.Вставить(Ключ, Запись);
	СтруктураДанных.Вставить("DSL_КэшМетаданных", Кэш);
	ЗаписатьДанныеДиалогаВРегистр(СсылкаДиалога, СтруктураДанных);
КонецПроцедуры

Функция УвеличитьСчетчикВосстановленийДиалога(СсылкаДиалога) Экспорт
	Если НЕ ЗначениеЗаполнено(СсылкаДиалога) Тогда
		Возврат 0;
	КонецЕсли;
	СтруктураДанных = ПолучитьДанныеДиалогаИзРегистра(СсылкаДиалога);
	...
	КонецЕсли;
	Счетчик = 0;
	Если СтруктураДанных.Свойство("Оркестратор_ЧислоВосстановлений") Тогда
		Счетчик = Число(СтруктураДанных.Оркестратор_ЧислоВосстановлений);
	КонецЕсли;
	Счетчик = Счетчик + 1;
	СтруктураДанных.Вставить("Оркестратор_ЧислоВосстановлений", Счетчик);
	ЗаписатьДанныеДиалогаВРегистр(СсылкаДиалога, СтруктураДанных);
	Возврат Счетчик;
КонецФункции
...
Процедура ДобавитьИзмененныеОбъекты(СсылкаДиалога, МассивСсылок) Экспорт
	...
	Если КоличествоДобавленных > 0 Тогда
		...
		СтрокиСообщения.Добавить("Созданные/измененные объекты:");
		Для Каждого СсылкаОбъекта Из ПолучитьИзмененныеОбъектыДиалога(СсылкаДиалога) Цикл
			СтрокиСообщения.Добавить("- " + Строка(СсылкаОбъекта));
		КонецЦикла;
		ДобавитьСообщениеВДиалог(
			СсылкаДиалога,
			Перечисления.ИИА_АвторСообщения.Система,
			Перечисления.ИИА_ТипСообщения.Текст,
			СтрСоединить(СтрокиСообщения, Символы.ПС),
			"",
			"",
			0
		);
	КонецЕсли;
	ПредставленияИзмененныхОбъектов = ПолучитьПредставленияИзмененныхОбъектовДиалога(СсылкаДиалога);
	НавигационныеСсылкиИзмененныхОбъектов = ПолучитьНавигационныеСсылкиИзмененныхОбъектовДиалога(СсылкаДиалога);
	ТекстИзмененныхОбъектов = СтрСоединить(ПредставленияИзмененныхОбъектов, Символы.ПС);
	ДополнительныеДанные = Новый Структура("ИзмененныеОбъектыПредставления,ИзмененныеОбъектыНавигационныеСсылки,ИзмененныеОбъектыТекст",
		ПредставленияИзмененныхОбъектов,
		НавигационныеСсылкиИзмененныхОбъектов,
		ТекстИзмененныхОбъектов);
	УведомитьОбОбновленииДиалога(СсылкаДиалога, "ИзмененныеОбъектыОбновлены", ДополнительныеДанные);

КонецПроцедуры

// Возвращает массив ссылок из табличной части ИзмененныеОбъекты диалога.
Функция ПолучитьИзмененныеОбъектыДиалога(СсылкаДиалога) Экспорт
	...
	Диалог = СсылкаДиалога.ПолучитьОбъект();
	Для Каждого СтрокаИзмененныхОбъектов Из Диалог.ИзмененныеОбъекты Цикл
		Если ЗначениеЗаполнено(СтрокаИзмененныхОбъектов.СсылкаНаОбъект)
			И Результат.Найти(СтрокаИзмененныхОбъектов.СсылкаНаОбъект) = Неопределено Тогда
			Результат.Добавить(СтрокаИзмененныхОбъектов.СсылкаНаОбъект);
		КонецЕсли;
	КонецЦикла;
...
// Возвращает массив представлений ссылок из табличной части ИзмененныеОбъекты диалога.
Функция ПолучитьПредставленияИзмененныхОбъектовДиалога(СсылкаДиалога) Экспорт

	Результат = Новый Массив;
	Для Каждого СсылкаНаОбъект Из ПолучитьИзмененныеОбъектыДиалога(СсылкаДиалога) Цикл
		Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
			Результат.Добавить(Строка(СсылкаНаОбъект));
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
...
// Возвращает массив навигационных ссылок для объектов диалога.
Функция ПолучитьНавигационныеСсылкиИзмененныхОбъектовДиалога(СсылкаДиалога) Экспорт
	...
	Для Каждого СсылкаНаОбъект Из ПолучитьИзмененныеОбъектыДиалога(СсылкаДиалога) Цикл
		...
		Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
			...
			КонецПопытки;
		КонецЕсли;
		Результат.Добавить(НавигационнаяСсылка);
	КонецЦикла;
	Возврат Результат;
...
Процедура УведомитьОбОбновленииДиалога(СсылкаДиалога, ТипСобытия = "НовоеСообщение", ДополнительныеДанные = Неопределено) Экспорт
	...
	Если ТипЗнч(ДополнительныеДанные) = Тип("Структура") Тогда
		Для Каждого КлючЗначение Из ДополнительныеДанные Цикл
			Если КлючЗначение.Ключ = "ИзмененныеОбъекты" Тогда
				Продолжить;
			КонецЕсли;
...
//
Процедура УстановитьДанныеЗапросаВХранилище(СсылкаДиалога, ДанныеЗапроса, КолонкиЗапроса = Неопределено) Экспорт
	...
	Попытка
		...
		Если КолонкиЗапроса <> Неопределено Тогда
			СтруктураДанных.Вставить("ПоследниеКолонкиЗапроса", КолонкиЗапроса);
		КонецЕсли;
	...
	КоличествоКолонок = 0;
	СписокКолонок = "";
	Если ТипЗнч(КолонкиЗапроса) = Тип("Массив") И КолонкиЗапроса.Количество() > 0 Тогда
		КоличествоКолонок = КолонкиЗапроса.Количество();
		Осталось = 10;
		Для Каждого ИмяКолонки Из КолонкиЗапроса Цикл
			Если Осталось <= 0 Тогда
				Прервать;
			КонецЕсли;
			Если ПустаяСтрока(СписокКолонок) Тогда
				СписокКолонок = Строка(ИмяКолонки);
			Иначе
				СписокКолонок = СписокКолонок + ", " + Строка(ИмяКолонки);
			КонецЕсли;
			Осталось = Осталось - 1;
		КонецЦикла;
...
Процедура УстановитьСостояниеЗапроса1СВХранилище(СсылкаДиалога, ДанныеЗапроса, ДанныеЗапроса1С = Неопределено) Экспорт
	...
	КолонкиЗапроса = Неопределено;
	Если ДанныеЗапроса1С <> Неопределено И ТипЗнч(ДанныеЗапроса1С) = Тип("Структура") И ДанныеЗапроса1С.Свойство("Колонки") Тогда
		КолонкиЗапроса = ДанныеЗапроса1С.Колонки;
	КонецЕсли;
	УстановитьДанныеЗапросаВХранилище(СсылкаДиалога, ДанныеЗапроса, КолонкиЗапроса);
...
Функция ВыполнитьЗапрос1СИзФормы(СсылкаДиалога, ТекстЗапроса, ПараметрыJSON = "") Экспорт
	...
	Если РезультатDSL.Свойство("КолонкиЗапросаДляТаблицы") Тогда
		ДанныеЗапроса1С.Вставить("Колонки", РезультатDSL.КолонкиЗапросаДляТаблицы);
	КонецЕсли;
