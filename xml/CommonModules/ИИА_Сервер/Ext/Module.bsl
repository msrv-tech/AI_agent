...
Функция ИнициализироватьПлан(СсылкаДиалога) Экспорт
	...
	ОтветИИ = ВызватьИИССистемнымСообщением(СсылкаДиалога, "Чат", ПромптПлана, "", "plan_json_array");
...
Функция ПолучитьОбщееКоличествоТокенов(СсылкаДиалога) Экспорт
	...
	// Если есть реквизит ПотраченоТокенов — используем его как источник истины (и синхронизируем при необходимости)
	Текущее = 0;
	Если ЗначениеЗаполнено(Диалог.ПотраченоТокенов) Тогда
		Текущее = Диалог.ПотраченоТокенов;
	КонецЕсли;
	
	// Миграция/самовосстановление: если раньше считали по сообщениям — поднимем значение до суммы
	Если Текущее < ОбщееКоличествоТокенов Тогда
		Диалог.ПотраченоТокенов = ОбщееКоличествоТокенов;
		Диалог.Записать();
		Текущее = Диалог.ПотраченоТокенов;
	КонецЕсли;
...
Процедура ДобавитьСообщениеВДиалог(СсылкаДиалога, Автор, ТипСообщения, Текст, ТекстКода = "", Статус = "", UsageTokens = 0, СтатусDSL = Неопределено) Экспорт
	...
	// Фиксируем потраченные токены на уровне диалога (накопительно)
	Если UsageTokens > 0 Тогда
		Текущее = 0;
		Если ЗначениеЗаполнено(Диалог.ПотраченоТокенов) Тогда
			Текущее = Диалог.ПотраченоТокенов;
		КонецЕсли;
		Диалог.ПотраченоТокенов = Текущее + UsageTokens;
	КонецЕсли;
...
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения, СистемныйПромпт = "", ExpectedResponseFormat = "dsl") Экспорт
	...
	Если ПустаяСтрока(СистемныйПромптДляВызова) Тогда
		Если ExpectedResponseFormat = "plan_json_array" Тогда
			СистемныйПромптДляВызова =
			"Ты - планировщик задач в 1С. Верни ТОЛЬКО валидный JSON массив строк (3-6 пунктов). " +
