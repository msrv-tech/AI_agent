...
//  Пользователь - Строка - имя пользователя, для которого получить настройки
	...
	Функция ПолучитьНастройкиПользователя(Знач Пользователь = "") Экспорт 
		...
		Результат.Вставить("ОкончаниеТокена", '00010101');
		Результат.Вставить("АвторизационныеДанные", "");
		Результат.Вставить("Scope", "");
		...
		Результат.Provider_Type = Выборка.Provider_Type;
		Результат.Токен = Выборка.Токен;
		Результат.ОкончаниеТокена = Выборка.ОкончаниеТокена;
		Результат.АвторизационныеДанные = Выборка.АвторизационныеДанные;
		Результат.Scope = Выборка.Scope;
...
Функция СгенерироватьПродолжениеВыполнения(СсылкаДиалога) Экспорт
	...
	Если Диалог.План.Количество() > 0 Тогда
		...
		Попытка
			...
			// ОЧИСТКА JSON (аналогично ИИА_DSL.ПроверитьDSL)
			ПозНачала = СтрНайти(ТекстОтвета, "{");
			ПозКонца = СтрНайти(ТекстОтвета, "}", НаправлениеПоиска.СКонца);
			Если ПозНачала > 0 И ПозКонца > ПозНачала Тогда
				ТекстОтвета = Сред(ТекстОтвета, ПозНачала, ПозКонца - ПозНачала + 1);
			КонецЕсли;
...
Процедура Gitsell_СохранитьAiProxyToken(Токен, Email = "") Экспорт
	...
	Запись.Provider_Type = "GITSELL_AIPROXY";
	Запись.Provider_ApiKey = Токен;
	Запись.Provider_BaseUrl = "https://gitsell.ru/api/v1"; // Стандартный URL
	
	// Если есть Email, можно сохранить его куда-то, но пока просто логируем факт сохранения
	// В текущей структуре регистра нет поля Email, поэтому просто игнорируем, если некуда писать
