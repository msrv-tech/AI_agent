...
Процедура ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, ЗаписьЛога) Экспорт
	...
	Попытка
		НаборЗаписей = РегистрыСведений.ИИА_Логи.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Диалог.Установить(СсылкаДиалога);
		НаборЗаписей.Прочитать();
		
		Если НаборЗаписей.Количество() = 0 Тогда
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.Диалог = СсылкаДиалога;
			НоваяЗапись.Лог = ТекстЗаписи;
		Иначе
			СуществующаяЗапись = НаборЗаписей[0];
			ТекущийЛог = СуществующаяЗапись.Лог;
			СуществующаяЗапись.Лог = ?(ПустаяСтрока(ТекущийЛог), ТекстЗаписи, ТекущийЛог + Символы.ПС + ТекстЗаписи);
		КонецЕсли;
		...
		Пока ПопыткаЗаписи < 3 Цикл
			...
			Исключение
				...
				Если ПопыткаЗаписи >= 3 Тогда
					ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "Ошибка при установке флага ТаблицаПоказана (пропуск): " + ТекстОшибки);
					Возврат;
				КонецЕсли;
			КонецПопытки;
		КонецЦикла;
