...
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения, СистемныйПромпт = "", ExpectedResponseFormat = "dsl") Экспорт
	...
	Если ПустаяСтрока(СистемныйПромптДляВызова) Тогда
		...
		ИначеЕсли ExpectedResponseFormat = "plan_and_next_dsl" Тогда
			СистемныйПромптДляВызова =
			"Ты - оркестратор плана. Верни ТОЛЬКО валидный JSON объект с полями updated_plan (массив) и next_step_dsl (DSL). " +
			"Никакого текста вне JSON. Без Markdown.";
		КонецЕсли;
	КонецЕсли;
