		...
		ИначеЕсли СтрНайти(ТекстОшибки, "не найден") > 0 Тогда
			...
			Если НЕ ПустаяСтрока(ТипОбъекта) Тогда
				...
				Если ПозКонца > 0 Тогда
					...
					ТекстОшибкиСКонтекстом = ТекстОшибкиСКонтекстом + Символы.ПС +
					"СОВЕТ: " + ТипОбъекта + " '" + ИмяОбъекта + "' не найден. ";
					
					Если Похожие.Количество() > 0 Тогда
						ТекстОшибкиСКонтекстом = ТекстОшибкиСКонтекстом + "Возможно, тебе подойдут эти объекты: " + СтрСоединить(Похожие, ", ") + ". ";
					КонецЕсли;
