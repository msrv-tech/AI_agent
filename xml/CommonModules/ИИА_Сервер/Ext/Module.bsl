...
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения, СистемныйПромпт = "", ExpectedResponseFormat = "dsl") Экспорт
	...
	ДобавитьЗаписьВЛогДиалога(
		СсылкаДиалога,
		"[LLM_RESPONSE_PARSED] CallId=" + CallId + ", ТипОтвета=" + ОтветИИ.ТипОтвета +
		UsageText +
		?(ОтветИИ.Свойство("DSL") И НЕ ПустаяСтрока(ОтветИИ.DSL), Символы.ПС + "[DSL]" + Символы.ПС + ОтветИИ.DSL, "") +
		?(ОтветИИ.Свойство("Текст") И НЕ ПустаяСтрока(ОтветИИ.Текст), Символы.ПС + "[Текст]" + Символы.ПС + ОтветИИ.Текст, "")
	);
