...
Функция ИзвлечьСущностиДляRAG(Знач ТекстЗапроса) Экспорт
	
	Промпт = ИИА_Промты.ПолучитьПромптЭкстрактораСущностей(ТекстЗапроса);
	
	// Используем легкий вызов без истории
	Сообщения = Новый Массив;
	Сообщение = Новый Структура("role, content", "user", Промпт);
	Сообщения.Добавить(Сообщение);
	
	// Получаем настройки по умолчанию
	Настройки = ПолучитьНастройкиПользователя(ПараметрыСеанса.ТекущийПользователь);
	Настройки.Вставить("ExpectedResponseFormat", "text");
	
	// Вызываем ИИ напрямую (через провайдера)
	ОтветИИ = ИИА_Провайдеры.ВызватьИИ(Сообщения, Настройки, 0.0); // Температура 0 для стабильности
	
	Если ОтветИИ.Успех И НЕ ПустаяСтрока(ОтветИИ.Текст) Тогда
