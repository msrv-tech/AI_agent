...
Функция ИнициализироватьПлан(СсылкаДиалога) Экспорт
	...
	ОтветИИ = ВызватьИИССистемнымСообщением(СсылкаДиалога, "Чат", ПромптПлана, ИИА_Промты.ПолучитьТекстПромптаПланировщика(), "plan_json_array");
...
Функция ОбновитьПланПоРезультатам(СсылкаДиалога) Экспорт
	...
	// В промпт кладем только "факты": dsl_system_result + краткий DSL. Без служебных/инструктивных системных текстов.
	ТекстРезультатов = "";
	...
	Для Индекс = КоличествоСообщений - 1 По Макс(0, КоличествоСообщений - 25) Цикл
		СтрокаСообщения = Диалог.Сообщения[Индекс];
		
		Если СтрокаСообщения.Автор = Перечисления.ИИА_АвторСообщения.Система Тогда
			
			ТекстСистемы = СокрЛП(СтрокаСообщения.Текст);
			Если ПустаяСтрока(ТекстСистемы) Тогда
				Продолжить;
			КонецЕсли;
			
			// Берем только результаты DSL (dsl_system_result) и ошибки
			Если (СтрНайти(ТекстСистемы, "dsl_system_result") > 0)
				ИЛИ (СтрокаСообщения.ТипСообщения = Перечисления.ИИА_ТипСообщения.Ошибка) Тогда
				
				Если СчетчикСистемных < 2 Тогда
					СчетчикСистемных = СчетчикСистемных + 1;
					
					Если СтрДлина(ТекстСистемы) > 1200 Тогда
						ТекстСистемы = Лев(ТекстСистемы, 1200) + "...(truncated)";
					КонецЕсли;
					
					МассивФрагментов.Вставить(0, ТекстСистемы);
				КонецЕсли;
				
			КонецЕсли;
			
		ИначеЕсли СтрокаСообщения.Автор = Перечисления.ИИА_АвторСообщения.ИИ 
			И НЕ ПустаяСтрока(СтрокаСообщения.ТекстКода) Тогда
			
			Если СчетчикDSL < 2 Тогда
				СчетчикDSL = СчетчикDSL + 1;
				
				ТекстDSL = СокрЛП(СтрокаСообщения.ТекстКода);
				Если СтрДлина(ТекстDSL) > 1200 Тогда
					ТекстDSL = Лев(ТекстDSL, 1200) + "...(truncated)";
				КонецЕсли;
		...
		Если СчетчикСистемных >= 2 И СчетчикDSL >= 2 Тогда
			Прервать;
		КонецЕсли;
	...
	// Вызываем ИИ
	ОтветИИ = ВызватьИИССистемнымСообщением(СсылкаДиалога, "Чат", ПромптОбновления, "", "plan_json_array");
...
Функция ИсправитьDSLЧерезИИ(СсылкаДиалога, ТекстОшибки, ИсходныйDSL) Экспорт
			...
			Если НЕ ПустаяСтрока(ТипОбъекта) Тогда
				...
				Если ПозКонца > 0 Тогда
					...
					Если Похожие.Количество() > 0 Тогда
						ТекстОшибкиСКонтекстом = ТекстОшибкиСКонтекстом + "ВМЕСТО НЕГО ИСПОЛЬЗУЙ ОДИН ИЗ ЭТИХ ОБЪЕКТОВ: " + СтрСоединить(Похожие, ", ") + ". ";
			...
			Если ПозТаблицы > 0 Тогда
				...
				Если ПозКонца > 0 Тогда
					...
					Если ПозТочки > 0 Тогда
						...
						// Формируем фильтр (берем первые 5-7 символов или до первой смены регистра/цифры)
						Фильтр = ИмяОбъекта;
						...
						Если Похожие.Количество() > 0 Тогда
							ТекстОшибкиСКонтекстом = ТекстОшибкиСКонтекстом + "ВМЕСТО НЕЁ ИСПОЛЬЗУЙ ОДИН ИЗ ЭТИХ ОБЪЕКТОВ: " + СтрСоединить(Похожие, ", ") + ". ";
							ТекстОшибкиСКонтекстом = ТекстОшибкиСКонтекстом + "Твой следующий шаг ОБЯЗАН быть RunQuery к одному из этих имен!";
						Иначе
							ТекстОшибкиСКонтекстом = ТекстОшибкиСКонтекстом + "Выполни GetMetadata с фильтром '" + Фильтр + "' для подтверждения.";
						КонецЕсли;
						...
						СсылкаДиалогаОбъект.ДополнительныеСвойства.Вставить("РекомендованныеОбъекты", Похожие);
	...
	Возврат ВызватьИИССистемнымСообщением(СсылкаДиалога, "Чат", ПромптИсправления, "", "dsl", Температура);
...
//
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения, СистемныйПромпт = "", ExpectedResponseFormat = "dsl", Температура = Неопределено) Экспорт
	...
	Настройки.Вставить("ExpectedResponseFormat", ExpectedResponseFormat);
	...
	ОтветИИ = ИИА_Провайдеры.ВызватьИИ(ТипСообщения, ТекстСистемногоСообщения, История, Настройки, СистемныйПромптДляВызова, Температура);
	...
	МодельДляЛога = "";
	Если Настройки.Свойство("Модель") И НЕ ПустаяСтрока(Настройки.Модель) Тогда
		МодельДляЛога = Настройки.Модель;
	Иначе
		МодельДляЛога = "auto";
	КонецЕсли;
	...
	ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, ТекстЛога);
...
Функция СгенерироватьSummary(СсылкаДиалога) Экспорт
	...
	Попытка
		...
		// Вызываем ИИ для генерации summary (без DSL-контракта)
		ОтветИИ = ВызватьИИССистемнымСообщением(СсылкаДиалога, "Чат", ПромптSummary, "", "text");
		...
		// Если модель вернула JSON (иногда бывает даже при запросе text), 
		// пытаемся извлечь поле "summary" или "text"
...
// Извлекает человекочитаемый текст summary из JSON-блока в тексте
//
// Параметры:
//  ИсходныйТекст - Строка - исходный текст сообщения
//
// Возвращаемое значение:
...
Функция ИзвлечьSummaryИзТекста(Знач ИсходныйТекст) Экспорт
	...
	// 1. Убираем технический заголовок
	...
	Если ПозНачалаJSON > 0 И ПозКонцаJSON > ПозНачалаJSON Тогда
		...
		// Пытаемся распарсить JSON и достать summary
		Попытка
			Чтение = Новый ЧтениеJSON;
			...
			Иначе
				ЧистыйТекст = ТекстБезЗаголовка; // Фолбек если структуры нет
			КонецЕсли;
		Исключение
			ЧистыйТекст = ТекстБезЗаголовка; // Фолбек при ошибке парсинга
		КонецПопытки;
	Иначе
...
// Выполняет финальную проверку и генерацию резюме (для фонового задания)
//
// Параметры:
//  СсылкаДиалога - СправочникСсылка.ИИА_Диалоги - ссылка на диалог
//
Процедура ВыполнитьФинальнуюПроверкуИSummary(СсылкаДиалога) Экспорт
	...
	УстановитьРезультатПроверкиВХранилище(СсылкаДиалога, РезультатПроверки.ПроверкаВыполнена, РезультатПроверки.Причина);
...
Процедура ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, ЗаписьЛога) Экспорт
	...
	Если Диалог.Метаданные().Реквизиты.Найти("Лог") <> Неопределено Тогда
		...
		// Режим отладки: перезаписываем файл полным содержимым реквизита Лог (путь из настроек ПутьКЛогуОтладки)
		Если РежимОтладкиJSON(Диалог.Пользователь) Тогда
			НастройкиОтладки = ПолучитьНастройкиПользователя(Диалог.Пользователь);
			ПутьЛога = "";
			Если НастройкиОтладки <> Неопределено И НастройкиОтладки.Свойство("ПутьКЛогуОтладки") Тогда
				ПутьЛога = СокрЛП(НастройкиОтладки.ПутьКЛогуОтладки);
			КонецЕсли;
			Если НЕ ПустаяСтрока(ПутьЛога) Тогда
...
Процедура УстановитьДанныеЗапросаВХранилище(СсылкаДиалога, ДанныеЗапроса)
	...
	// Новый набор данных — таблица ещё не показана
...
Функция ПолучитьНепоказанныеДанныеЗапроса(СсылкаДиалога) Экспорт
	...
	Попытка
		СтруктураДанных = Диалог.ХранилищеЗначения.Получить();
	Исключение
		ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "Ошибка чтения ХранилищеЗначения для показа таблицы: " + ОписаниеОшибки());
		Возврат Неопределено;
	КонецПопытки;
...
Функция ПроверитьРезультатВыполненияЗадачи(СсылкаДиалога) Экспорт
	...
	РезультатВызова = ВызватьИИССистемнымСообщением(СсылкаДиалога, "Чат", ПромптПроверки, "", "dsl");
...
Функция СгенерироватьПродолжениеВыполнения(СсылкаДиалога) Экспорт
	...
	Если Диалог.План.Количество() > 0 Тогда
		...
		// 3. Вызов ИИ
		ОтветИИ = ВызватьИИССистемнымСообщением(СсылкаДиалога, "Запрос", Промпт, "", "plan_and_next_dsl");
		...
		Если РезультатКомби.Свойство("updated_plan") И ТипЗнч(РезультатКомби.updated_plan) = Тип("Массив") Тогда
			Диалог = СсылкаДиалога.ПолучитьОбъект();
			ИзмененияПлана = Новый Массив;
			Для Каждого ПунктОбновления Из РезультатКомби.updated_plan Цикл
				Для Каждого СтрокаПлана Из Диалог.План Цикл
					Если СтрокаПлана.Задача = ПунктОбновления.Задача Тогда
						СтарВып = СтрокаПлана.Выполнена;
						СтарРез = СтрокаПлана.Результат;
						СтрокаПлана.Выполнена = ПунктОбновления.Выполнена;
						СтрокаПлана.Результат = ПунктОбновления.Результат;
						
						Если (СтарВып <> СтрокаПлана.Выполнена) ИЛИ (СтарРез <> СтрокаПлана.Результат) Тогда
							ИзмененияПлана.Добавить(
								"- " + СтрокаПлана.Задача +
								" | Выполнена: " + ?(СтарВып, "true", "false") + " -> " + ?(СтрокаПлана.Выполнена, "true", "false") +
								" | Результат: " + ?(ПустаяСтрока(СтарРез), "''", "'" + СтарРез + "'") + " -> " + ?(ПустаяСтрока(СтрокаПлана.Результат), "''", "'" + СтрокаПлана.Результат + "'")
							);
						КонецЕсли;
						Прервать;
					КонецЕсли;
			...
			Если ИзмененияПлана.Количество() > 0 Тогда
				ТекстИзменений = "[План] Обновление статусов (" + Формат(ИзмененияПлана.Количество(), "ЧН=0") + "):";
				Для Каждого СтрИзм Из ИзмененияПлана Цикл
					ТекстИзменений = ТекстИзменений + Символы.ПС + СтрИзм;
				КонецЦикла;
				ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, ТекстИзменений);
			КонецЕсли;
		КонецЕсли;
		...
		Если РезультатКомби.Свойство("next_step_dsl") Тогда
			...
			// Статус сообщения = имя текущего пункта плана
			ТекущаяЗадачаПлана = "";
			Для Каждого СтрокаПлана Из Диалог.План Цикл
				Если НЕ СтрокаПлана.Выполнена Тогда
					ТекущаяЗадачаПлана = СтрокаПлана.Задача;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			ТекстСтатусаСообщения = ?(ПустаяСтрока(ТекущаяЗадачаПлана), "Следующий шаг выполнения по плану", ТекущаяЗадачаПлана);
			...
			ДобавитьСообщениеВДиалог(
				...
				ТекстDSL,
				ТекстСтатусаСообщения,
				0
			);
			
			// ВАЖНО: выполняем next_step_dsl сразу, иначе оркестратор зацикливается на генерации,
			// т.к. "Факты последних действий" остаются пустыми.
			ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[Система] Автовыполнение next_step_dsl...");
			РезультатАвтоDSL = ВыполнитьDSLСценарийССообщениями(СсылкаДиалога, ТекстDSL);
			
			// Детерминированно продвигаем план: отмечаем текущий (первый невыполненный) пункт как выполненный по факту исполнения next_step_dsl
			Попытка
				Диалог = СсылкаДиалога.ПолучитьОбъект();
				Для Каждого СтрокаПлана Из Диалог.План Цикл
					Если НЕ СтрокаПлана.Выполнена Тогда
						...
						СтрокаПлана.Выполнена = ?(РезультатАвтоDSL.Свойство("Успех") И РезультатАвтоDSL.Успех, Истина, Ложь);
						
						НовыйРез = "";
						Если РезультатАвтоDSL.Свойство("Сообщение") И НЕ ПустаяСтрока(Строка(РезультатАвтоDSL.Сообщение)) Тогда
							НовыйРез = Строка(РезультатАвтоDSL.Сообщение);
						Иначе
							НовыйРез = ?(СтрокаПлана.Выполнена, "Выполнено", "Ошибка");
						КонецЕсли;
						
						Если СтрДлина(НовыйРез) > 300 Тогда
							НовыйРез = Лев(НовыйРез, 300) + "...(truncated)";
						КонецЕсли;
						
						СтрокаПлана.Результат = НовыйРез;
						Диалог.Записать();
						
						ДобавитьЗаписьВЛогДиалога(
							СсылкаДиалога,
							"[План] Автообновление по выполненному next_step_dsl:" + Символы.ПС +
							"- " + СтрокаПлана.Задача +
							" | Выполнена: " + ?(СтарВып, "true", "false") + " -> " + ?(СтрокаПлана.Выполнена, "true", "false") +
							" | Результат: " + ?(ПустаяСтрока(СтарРез), "''", "'" + СтарРез + "'") + " -> " + ?(ПустаяСтрока(СтрокаПлана.Результат), "''", "'" + СтрокаПлана.Результат + "'")
						);
						
						// Отправляем уведомление клиенту о завершении шага
						ДанныеУведомления = Новый Структура;
						ДанныеУведомления.Вставить("Диалог", СсылкаДиалога);
						ДанныеУведомления.Вставить("ТипСобытия", "ШагВыполнен");
						УведомленияКлиента.ОтправитьУведомление("СообщениеПереписки", ДанныеУведомления);
				...
				КонецЦикла;
			Исключение
				ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[План] Ошибка автообновления по результату next_step_dsl: " + ОписаниеОшибки());
			КонецПопытки;
	...
	Иначе
		...
		ПромптПродолжения = ИИА_Промты.ПолучитьПромптПродолженияЗадачи(Причина, ДопИнструкция);
	КонецЕсли;
	...
	ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ПромптПродолжения, "", "dsl");
