...
//
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения, СистемныйПромпт = "", ExpectedResponseFormat = "dsl", Температура = Неопределено, Знач БезRAG = Ложь) Экспорт
	...
	ПараметрыПользователя.Вставить("БезRAG", БезRAG);
