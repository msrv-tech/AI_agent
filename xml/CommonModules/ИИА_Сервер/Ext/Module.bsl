...
Функция ИзвлечьСущностиДляRAG(Знач ТекстЗапроса, Знач СсылкаДиалога = Неопределено) Экспорт
	...
	ПараметрыИИ.Вставить("БезRAG", Истина);
...
Процедура ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, ЗаписьЛога) Экспорт
	...
	// Если ссылка не передана или это не ссылка на справочник, ничего не делаем
	...
	Если Диалог.Метаданные().Реквизиты.Найти("Лог") <> Неопределено Тогда
		...
		Если РежимОтладкиJSON(Диалог.Пользователь) Тогда
			...
			Если НЕ ПустаяСтрока(ПутьЛога) Тогда
				...
				Исключение
					// Если файл занят другим процессом (например, открыт в редакторе с блокировкой),
					// просто игнорируем ошибку записи в файл, так как основной лог в БД уже записан.
				КонецПопытки;
			КонецЕсли;
		КонецЕсли;
