...
Функция МаскироватьОпасныеФрагментыЗапросаДляЛога(Текст)
	Результат = Строка(Текст);
	Результат = СтрЗаменить(Результат, "ТЕКУЩАЯДАТА(", "ТЕКУЩАЯДАТА[masked](");
	Результат = СтрЗаменить(Результат, "ТекущаяДата(", "ТекущаяДата[masked](");
	Результат = СтрЗаменить(Результат, "текущаядата(", "текущаядата[masked](");
	Результат = СтрЗаменить(Результат, "НАЧАЛОПЕРИОДА(", "НАЧАЛОПЕРИОДА[masked](");
	Результат = СтрЗаменить(Результат, "КОНЕЦПЕРИОДА(", "КОНЕЦПЕРИОДА[masked](");
	Возврат Результат;
КонецФункции
...
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения, СистемныйПромпт = "", ExpectedResponseFormat = "dsl", Температура = Неопределено, Знач ИспользоватьRAG = Ложь, Знач АвтоВыполнятьDSL = Истина) Экспорт
	...
	МодельДляЛога = ?(ПараметрыПользователя.Свойство("Модель") И НЕ ПустаяСтрока(ПараметрыПользователя.Модель), ПараметрыПользователя.Модель, ИИА_GitsellСервер.МодельПоУмолчанию());
	...
	ТекстЛога = ТекстЛога + Символы.ПС + МаскироватьОпасныеФрагментыЗапросаДляЛога(ИИА_DialogLog.СжатьJSONВСтроку(ОтветИИ.Промпт));
	...
	КонецЕсли;
	RawTextДляЛога = МаскироватьОпасныеФрагментыЗапросаДляЛога(RawTextДляЛога);
	...
	Если НЕ ПустаяСтрока(НормализованныйJSON) И СокрЛП(НормализованныйJSON) <> СокрЛП(RawText) Тогда
		ИИА_DialogLog.ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[LLM_RESPONSE_NORMALIZED_JSON] CallId=" + CallId + Символы.ПС + МаскироватьОпасныеФрагментыЗапросаДляЛога(ИИА_DialogLog.СжатьJSONВСтроку(НормализованныйJSON)), "LLM_RESPONSE_NORMALIZED_JSON", CallId);
	КонецЕсли;
	...
	DSLДляЛога = ?(ОтветИИ.Свойство("DSL") И НЕ ПустаяСтрока(ОтветИИ.DSL), Символы.ПС + "[DSL]" + Символы.ПС + МаскироватьОпасныеФрагментыЗапросаДляЛога(ИИА_DialogLog.СжатьJSONВСтроку(ОтветИИ.DSL)), "");
	ToolДляЛога = "";
	...
	Если ОтветИИ.ТипОтвета = "Ошибка" ИЛИ ОтветИИ.ТипОтвета = "ОшибкаКонтракта" Тогда
		Если ОтветИИ.Свойство("RawModelText") И НЕ ПустаяСтрока(Строка(ОтветИИ.RawModelText)) Тогда
			ИИА_Сервер.ДобавитьСообщениеВДиалог(СсылкаДиалога, Перечисления.ИИА_АвторСообщения.ИИ, Перечисления.ИИА_ТипСообщения.Текст, МаскироватьОпасныеФрагментыЗапросаДляЛога(Строка(ОтветИИ.RawModelText)), "", "", UsageTokens, Неопределено, "", СкрытоеСлужебноеОтвета);
	...
	Иначе
		...
		ТекстКодаОтвета = "";
	КонецЕсли;
	Если СкрытоеСлужебноеОтвета Тогда
		ТекстОтвета = МаскироватьОпасныеФрагментыЗапросаДляЛога(ТекстОтвета);
		ТекстКодаОтвета = МаскироватьОпасныеФрагментыЗапросаДляЛога(ТекстКодаОтвета);
	КонецЕсли;
...
Функция ОтправитьСообщениеСервера(СсылкаДиалога, ТипСообщения, ТекстПользователя) Экспорт
	...
	Если Диалог.ТипДиалога = Перечисления.ИИА_ТипДиалога.Агент
		ИЛИ Диалог.ТипДиалога = Перечисления.ИИА_ТипДиалога.Запрос1С
		ИЛИ Диалог.ТипДиалога = Перечисления.ИИА_ТипДиалога.РаспознаваниеДокументов Тогда
		Диалог = СсылкаДиалога.ПолучитьОбъект();
			...
			Если ЗначениеЗаполнено(СсылкаДиалога) Тогда
				Если РезультатDSL.Свойство("ПоследнийЗапрос1С") Тогда
					Если РезультатDSL.Свойство("КолонкиЗапросаДляТаблицы") Тогда
						РезультатDSL.ПоследнийЗапрос1С.Вставить("Колонки", РезультатDSL.КолонкиЗапросаДляТаблицы);
					КонецЕсли;
				...
				Иначе
					ИИА_Сервер.УстановитьДанныеЗапросаВХранилище(
						СсылкаДиалога,
						РезультатDSL.ДанныеЗапросаДляТаблицы,
						?(РезультатDSL.Свойство("КолонкиЗапросаДляТаблицы"), РезультатDSL.КолонкиЗапросаДляТаблицы, Неопределено)
					);
				КонецЕсли;
			КонецЕсли;
