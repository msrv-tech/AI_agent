...
Процедура ЗаписатьСтатистику(DF, ВсегоЧанков)
	...
	// Чтобы избежать дублей, если один и тот же токен попал в DF несколько раз (хотя это Соответствие)
	// или если в наборе уже есть такие токены.
	Для каждого Пара Из DF Цикл
		...
		// Используем натуральный логарифм (Log в 1С)
		idfVal = Log((ВсегоЧанков - dfVal + 0.5) / (dfVal + 0.5) + 1);
		
		НоваяЗапись = НаборСтатистики.Добавить();
		НоваяЗапись.Токен = Лев(Токен, 64); // Ограничение длины по метаданным
		НоваяЗапись.DF = dfVal;
		НоваяЗапись.IDF = idfVal;
		
		// Записываем порциями, чтобы не переполнять память и избежать проблем с уникальностью в больших наборах
