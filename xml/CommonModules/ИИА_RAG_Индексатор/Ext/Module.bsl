...
Процедура ОчиститьИндекс()
	...
	Попытка
		РегистрыСведений.ИИА_ЭмбеддингиЧанков.СоздатьНаборЗаписей().Записать();
	Исключение
		// Регистр может не существовать в старых версиях
	КонецПопытки;
...
Процедура ЗаписатьЧанк(Тип, Имя, Синоним, Путь, Суффикс, ТекстИсх, DF, ВсегоЧанков, СтопСлова)
	...
	// Индексация эмбеддинга чанка (Yandex Embeddings, если доступны)
	Попытка
		Если ИИА_RAG_Эмбеддинги.ЭмбеддингиДоступны() Тогда
			ИИА_RAG_Эмбеддинги.ИндексироватьЧанк(КлючЧанка, НормТекст);
		КонецЕсли;
	Исключение
		// Ошибка эмбеддинга не должна прерывать основную индексацию
	КонецПопытки;
