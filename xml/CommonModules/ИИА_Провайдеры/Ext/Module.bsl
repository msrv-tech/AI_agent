...
Функция ВыполнитьHTTPИнструмент(БазовыйДомен, URLПуть, Заголовки, JSONТело, Таймаут = 30, МаксПопыток = 3, Порт = 443, ИспользоватьSSL = Истина)
	...
	Для ПопыткаНомер = 1 По МаксПопыток Цикл
		Попытка
			Если ИспользоватьSSL Тогда
				HTTPСоединение = Новый HTTPСоединение(БазовыйДомен, Порт, , , , Таймаут, Новый ЗащищенноеСоединениеOpenSSL());
			Иначе
				HTTPСоединение = Новый HTTPСоединение(БазовыйДомен, Порт, , , , Таймаут);
			КонецЕсли;
...
Функция ВызватьGitsellAiProxy(ТипСообщения, ТекстПользователя, История, ПараметрыИИ, СистемныйПромпт = "", Температура = Неопределено)
	...
	// Определяем протокол
	ИспользоватьSSL = СтрНайти(URLИзНастроек, "http://") <> 1;
	...
	// Извлекаем порт из домена (если указан, например 192.168.11.150:1234)
	ПозДвоеточияПорта = СтрНайти(БазовыйДомен, ":");
	Если ПозДвоеточияПорта > 0 Тогда
		Порт = Число(Сред(БазовыйДомен, ПозДвоеточияПорта + 1));
		БазовыйДомен = Лев(БазовыйДомен, ПозДвоеточияПорта - 1);
	Иначе
		Порт = ?(ИспользоватьSSL, 443, 80);
	КонецЕсли;
	...
	РезультатHTTP = ВыполнитьHTTPИнструмент(БазовыйДомен, URLПуть, Заголовки, JSONТело, 30, 3, Порт, ИспользоватьSSL);
