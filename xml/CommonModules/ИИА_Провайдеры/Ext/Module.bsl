...
Функция ВызватьИИ(ТипСообщения, ТекстПользователя, История, Настройки, СистемныйПромпт = "") Экспорт
	...
	Если ВРег(ТипПровайдера) = "GIGACHAT" ИЛИ ВРег(ТипПровайдера) = "ГИГАЧАТ" ИЛИ ПустаяСтрока(ТипПровайдера) Тогда
		...
		Результат = ИИА_Гигачат.ВызватьГигачат(ТипСообщения, ТекстПользователя, История, Настройки, СистемныйПромпт);
		
	ИначеЕсли ВРег(ТипПровайдера) = "GITSELL_AIPROXY" Тогда
...
Функция ВызватьGitsellAiProxy(ТипСообщения, ТекстПользователя, История, Настройки, СистемныйПромпт = "")
	...
	ТелоЗапроса.Вставить("model", "auto"); // Используем auto по умолчанию
	...
	Попытка
		...
		Иначе
			...
			Если Ответ.КодСостояния = 401 Тогда
				Результат.Текст = "Ошибка 401: Требуется авторизация. Токен отозван или невалиден. Нажмите 'Войти' повторно.";
			Иначе
				Результат.Текст = "Ошибка API AI Proxy (" + Ответ.КодСостояния + "): " + ТекстОтвета;
			КонецЕсли;
	...
	Исключение
		Результат.Текст = "Ошибка соединения с AI Proxy: " + ОписаниеОшибки();
