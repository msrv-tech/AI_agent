...
Функция ВызватьGitsellAiProxy(ТипСообщения, ТекстПользователя, История, ПараметрыИИ, СистемныйПромпт = "", Температура = Неопределено)
	...
	БезRAG = Ложь;
	Если ПараметрыИИ.Свойство("БезRAG") Тогда
		БезRAG = ПараметрыИИ.БезRAG;
	КонецЕсли;
	
	Промпт = ИИА_Промты.СформироватьПромпт(ТипДиалога, ТипСообщения, ТекстПользователя, История, СистемныйПромпт, СсылкаДиалога, БезRAG);
