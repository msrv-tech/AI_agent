...
// Ищет похожие объекты в метаданных (fuzzy search)
//
// Параметры:
...
//
// Возвращаемое значение:
//  Массив - массив имен похожих объектов
//
Функция НайтиПохожиеОбъекты(ИмяДляПоиска, ТипМетаданных) Экспорт
	
	Результат = Новый Массив;
	КлючевоеСлово = ВРег(Лев(ИмяДляПоиска, 6)); // Берем начало имени
	...
	Если Коллекция <> Неопределено Тогда
		Для Каждого ОбъектМетаданных Из Коллекция Цикл
			// Простой поиск по вхождению ключевого слова
			Если СтрНайти(ВРег(ОбъектМетаданных.Имя), КлючевоеСлово) > 0 
				ИЛИ СтрНайти(ВРег(ОбъектМетаданных.Синоним), КлючевоеСлово) > 0 Тогда
				Результат.Добавить(ОбъектМетаданных.Имя);
			КонецЕсли;
			
			Если Результат.Количество() >= 5 Тогда // Ограничиваем количество подсказок
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
