...
Функция НайтиПохожиеОбъекты(ИмяДляПоиска, ТипМетаданных) Экспорт
	...
	Для Каждого СтрокаRAG Из РезультатыRAG Цикл
		...
		Если НЕ ПустаяСтрока(ТипМетаданных) Тогда
			Если ТипМетаданных = "Справочник" И НЕ СтрНачинаетсяС(СтрокаRAG.ПолноеИмя, "Справочник.") Тогда
				Продолжить;
			ИначеЕсли ТипМетаданных = "Документ" И НЕ СтрНачинаетсяС(СтрокаRAG.ПолноеИмя, "Документ.") Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
