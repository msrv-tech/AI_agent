...
Функция ПолучитьСписокСправочников(Фильтр = "") Экспорт
	...
	Для Каждого МетаданныеСправочника Из Метаданные.Справочники Цикл
		...
		Если НЕ ПустаяСтрока(Фильтр) Тогда
			Если СтрНайти(ВРег(МетаданныеСправочника.Имя), ФильтрВРег) = 0 
				И СтрНайти(ВРег(МетаданныеСправочника.Синоним), ФильтрВРег) = 0 Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьСписокДокументов(Фильтр = "") Экспорт
	...
	Для Каждого МетаданныеДокумента Из Метаданные.Документы Цикл
		...
		Если НЕ ПустаяСтрока(Фильтр) Тогда
			Если СтрНайти(ВРег(МетаданныеДокумента.Имя), ФильтрВРег) = 0 
				И СтрНайти(ВРег(МетаданныеДокумента.Синоним), ФильтрВРег) = 0 Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
Функция НайтиПохожиеОбъекты(ИмяДляПоиска, ТипМетаданных) Экспорт
		...
		Для Каждого ОбъектМетаданных Из Коллекция Цикл
			...
			Для Каждого ТекущийЗапрос Из МассивЗапросов Цикл
				...
				Если СтрНайти(ИмяВРег, ТекущийЗапрос) > 0 ИЛИ СтрНайти(СинонимВРег, ТекущийЗапрос) > 0 Тогда
					ТекущееСходство = Макс(ТекущееСходство, 0.8);
				КонецЕсли;
			...
			Если МаксСходствоДляОбъекта > 0.3 Тогда
				СписокСходства.Добавить(ИмяОбъекта, , , МаксСходствоДляОбъекта);
			КонецЕсли;
	...
	СписокСходства.СортироватьПоЗначению(НаправлениеСортировки.Убывание);
