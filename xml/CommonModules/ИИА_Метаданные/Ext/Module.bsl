...
Функция НайтиПохожиеОбъекты(ИмяДляПоиска, ТипМетаданных) Экспорт
	...
	Если Коллекция <> Неопределено Тогда
		Для Каждого ОбъектМетаданных Из Коллекция Цикл
			...
			СходствоИмя = ВычислитьКоэффициентСходства(ПоисковыйЗапрос, ВРег(ИмяОбъекта));
			СходствоСиноним = ВычислитьКоэффициентСходства(ПоисковыйЗапрос, ВРег(СинонимОбъекта));
			
			МаксСходство = Макс(СходствоИмя, СходствоСиноним);
			
			// Если сходство выше порога или есть прямое вхождение
			Если МаксСходство > 0.3 ИЛИ СтрНайти(ВРег(ИмяОбъекта), ПоисковыйЗапрос) > 0 ИЛИ СтрНайти(ВРег(СинонимОбъекта), ПоисковыйЗапрос) > 0 Тогда
				СписокСходства.Добавить(ИмяОбъекта, , , МаксСходство);
			КонецЕсли;
