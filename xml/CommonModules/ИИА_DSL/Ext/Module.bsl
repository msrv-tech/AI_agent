...
Функция ИзвлечьПоляИзТекстаЗапроса(ТекстЗапроса)
	...
	ТекстВерх = ВРег(ТекстЗапроса);
	ПозВыбрать = СтрНайти(ТекстВерх, "ВЫБРАТЬ");
	ПозИз = СтрНайти(ТекстВерх, " ИЗ ");
	Если ПозВыбрать = 0 Или ПозИз = 0 Или ПозИз <= ПозВыбрать Тогда
		Возврат Результат;
	КонецЕсли;
	
	СписокВыбора = Сред(ТекстЗапроса, ПозВыбрать + 7, ПозИз - (ПозВыбрать + 7));
	Колонки = СтрРазделить(СписокВыбора, ",", Ложь);
	
	Для Каждого Колонка Из Колонки Цикл
		Текущая = СокрЛП(Колонка);
		Если ПустаяСтрока(Текущая) Тогда
			Продолжить;
		КонецЕсли;
		Если СтрНайти(ВРег(Текущая), "КАК ") > 0 Тогда
			ЧастиКолонки = СтрРазделить(Текущая, "КАК", Ложь);
			Если ЧастиКолонки.Количество() > 0 Тогда
				Текущая = СокрЛП(ЧастиКолонки[0]);
			КонецЕсли;
		КонецЕсли;
		Если СтрНайти(Текущая, "(") > 0 ИЛИ Текущая = "*" Тогда
			Продолжить;
		КонецЕсли;
		ПозТочки = СтрНайти(Текущая, ".");
		Если ПозТочки > 0 Тогда
			Текущая = Сред(Текущая, ПозТочки + 1);
		КонецЕсли;
		Текущая = СокрЛП(Текущая);
		Если НЕ ПустаяСтрока(Текущая) И Результат.Найти(Текущая) = Неопределено Тогда
...
Функция ВалидироватьШаг(Шаг, НомерШага)
	...
	Если Действие = "CreateReference" Тогда
		...
		ИмяСправочника = НормализоватьИмяОбъектаМетаданных(Шаг.object_name, "Справочник");
		Если НЕ ИИА_Метаданные.СуществуетСправочник(ИмяСправочника) Тогда
			Результат.Успех = Ложь;
			Результат.Сообщение = "Шаг " + Формат(НомерШага, "ЧН=0") + ": справочник '" + ИмяСправочника + "' не найден";
			Возврат Результат;
		КонецЕсли;
	...
	Попытка
		...
		Для Каждого РеквизитОбъекта Из ИнформацияОбОбъекте.Реквизиты Цикл
			Если ТипЗнч(РеквизитОбъекта) = Тип("Структура") И РеквизитОбъекта.Свойство("Имя") Тогда
				Если Лев(ВРег(РеквизитОбъекта.Имя), 6) <> "УДАЛИТ" Тогда
					WhitelistПоля.Добавить(РеквизитОбъекта.Имя);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
