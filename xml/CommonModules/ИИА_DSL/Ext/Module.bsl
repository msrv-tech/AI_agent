...
Функция ПолучитьРеестрКонтрактовDSL() Экспорт
	...
	Реестр.Вставить("ImportPriceList", Новый Структура("name,input_schema,idempotent,requires_write,capability", "ImportPriceList", "file_path,price_type?", Ложь, Истина, "data.write.reference"));
	Реестр.Вставить("ParseFileForContractor", Новый Структура("name,input_schema,idempotent,requires_write,capability", "ParseFileForContractor", "file_path", Истина, Ложь, "data.read"));
...
Функция ВыполнитьШаг(Шаг, КонтекстВыполнения) Экспорт
	...
	Попытка
		...
		ИначеЕсли Действие = "ImportPriceList" Тогда

			Результат = ВыполнитьImportPriceList(ШагСПараметрами, КонтекстВыполнения);

		ИначеЕсли Действие = "ParseFileForContractor" Тогда

			Результат = ВыполнитьParseFileForContractor(ШагСПараметрами, КонтекстВыполнения);
...
// =============================================================================
// Кейс 11: Импорт прайс-листа из Excel-файла
// =============================================================================

Функция ВыполнитьImportPriceList(Шаг, КонтекстВыполнения)

	Результат = Новый Структура("Успех,Сообщение", Ложь, "");

	ПутьКФайлу = "";
	Если Шаг.Свойство("file_path") Тогда
		ПутьКФайлу = СокрЛП(Строка(Шаг.file_path));
	КонецЕсли;

	Если ПустаяСтрока(ПутьКФайлу) Тогда
		Результат.Сообщение = "Не указан путь к файлу (file_path).";
		Возврат Результат;
	КонецЕсли;

	ВидЦеныНаименование = "Оптовая";
	Если Шаг.Свойство("price_type") Тогда
		ВидЦеныНаименование = СокрЛП(Строка(Шаг.price_type));
	КонецЕсли;
	...
	Попытка
		...
		ТабДокумент.Прочитать(ПутьКФайлу, СпособЧтенияТабличногоДокумента.Значение);
	Исключение
		Результат.Сообщение = "Ошибка чтения файла Excel: " + ОписаниеОшибки();
		Возврат Результат;
	КонецПопытки;

	Если ТабДокумент.ВысотаТаблицы < 2 Тогда
		Результат.Сообщение = "Файл пустой или содержит только заголовок.";
		Возврат Результат;
	КонецЕсли;
	...
	Попытка
		ВидЦены = Справочники.ВидыЦен.НайтиПоНаименованию(ВидЦеныНаименование, Истина);
	Исключение
		// Справочник может называться иначе
	КонецПопытки;

	Если ВидЦены = Неопределено ИЛИ ВидЦены.Пустая() Тогда
		Результат.Сообщение = "Вид цены '" + ВидЦеныНаименование + "' не найден в справочнике ВидыЦен.";
		Возврат Результат;
	КонецЕсли;
	...
	Для НомерСтроки = 2 По ТабДокумент.ВысотаТаблицы Цикл
		...
		Наименование = СокрЛП(ТабДокумент.Область(НомерСтроки, 1).Текст);
		ЦенаТекст = СокрЛП(ТабДокумент.Область(НомерСтроки, 2).Текст);

		Если ПустаяСтрока(Наименование) ИЛИ ПустаяСтрока(ЦенаТекст) Тогда
			Продолжить;
		КонецЕсли;

		Попытка
			ЦенаЧисло = Число(СтрЗаменить(ЦенаТекст, ",", "."));
		Исключение
			КоличествоОшибок = КоличествоОшибок + 1;
			ТекстОшибок = ТекстОшибок + "Строка " + Строка(НомерСтроки) + ": некорректная цена '" + ЦенаТекст + "'" + Символы.ПС;
			Продолжить;
		КонецПопытки;
		...
		Если Номенклатура.Пустая() Тогда
			КоличествоОшибок = КоличествоОшибок + 1;
			ТекстОшибок = ТекстОшибок + "Строка " + Строка(НомерСтроки) + ": номенклатура '" + Наименование + "' не найдена" + Символы.ПС;
			Продолжить;
		КонецЕсли;

		// Запись цены в регистр
		Попытка
			МенеджерЗаписи = РегистрыСведений.ЦеныНоменклатуры.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.Период = ТекущаяДата();
			МенеджерЗаписи.Номенклатура = Номенклатура;
			МенеджерЗаписи.ВидЦены = ВидЦены;
			МенеджерЗаписи.Цена = ЦенаЧисло;
			МенеджерЗаписи.Записать();
			КоличествоЗагружено = КоличествоЗагружено + 1;
		Исключение
			КоличествоОшибок = КоличествоОшибок + 1;
			ТекстОшибок = ТекстОшибок + "Строка " + Строка(НомерСтроки) + ": ошибка записи — " + ОписаниеОшибки() + Символы.ПС;
		КонецПопытки;

	КонецЦикла;

	Результат.Успех = Истина;
	Результат.Сообщение = "Импорт прайс-листа завершён. Загружено: " + Строка(КоличествоЗагружено) + ", ошибок: " + Строка(КоличествоОшибок);
	Если НЕ ПустаяСтрока(ТекстОшибок) Тогда
		Результат.Сообщение = Результат.Сообщение + Символы.ПС + "Детали ошибок:" + Символы.ПС + ТекстОшибок;
	КонецЕсли;
...
// =============================================================================
// Кейс 11: Разбор файла для извлечения реквизитов контрагента
// Поддерживает xlsx, xls, txt, csv, docx, pdf
// =============================================================================

Функция ВыполнитьParseFileForContractor(Шаг, КонтекстВыполнения)
	...
	Если Расширение = ".xlsx" ИЛИ Расширение = ".xls" Тогда
		Попытка
			ТабДок = Новый ТабличныйДокумент;
			ТабДок.Прочитать(ПутьКФайлу, СпособЧтенияТабличногоДокумента.Значение);
			Для Стр = 1 По Мин(ТабДок.ВысотаТаблицы, 100) Цикл
				Для Кол = 1 По Мин(ТабДок.ШиринаТаблицы, 20) Цикл
					Ячейка = СокрЛП(ТабДок.Область(Стр, Кол).Текст);
					Если НЕ ПустаяСтрока(Ячейка) Тогда
						ТекстФайла = ТекстФайла + Ячейка + " ";
					КонецЕсли;
				КонецЦикла;
				ТекстФайла = ТекстФайла + Символы.ПС;
			КонецЦикла;
		Исключение
			Результат.Сообщение = "Ошибка чтения Excel: " + ОписаниеОшибки();
			Возврат Результат;
		КонецПопытки;
	...
	ИначеЕсли Расширение = ".txt" ИЛИ Расширение = ".csv" Тогда
		...
		Исключение
			Результат.Сообщение = "Ошибка чтения текстового файла: " + ОписаниеОшибки();
			Возврат Результат;
		КонецПопытки;
	...
	ИначеЕсли Расширение = ".docx" Тогда
		Попытка
			...
			COMОбъект.Visible = Ложь;
			Документ = COMОбъект.Documents.Open(ПутьКФайлу, , Истина);
			ТекстФайла = Документ.Content.Text;
			Документ.Close(Ложь);
			COMОбъект.Quit();
		Исключение
			Попытка
			...
			Исключение
			КонецПопытки;
			Результат.Сообщение = "Ошибка чтения DOCX (требуется COM Word): " + ОписаниеОшибки();
			Возврат Результат;
		КонецПопытки;

	ИначеЕсли Расширение = ".pdf" Тогда
		Результат.Сообщение = "Формат PDF: сохраните содержимое в .txt или .docx и повторите запрос.";
		Возврат Результат;

	Иначе
		Результат.Сообщение = "Неподдерживаемый формат файла: " + Расширение + ". Поддерживаются: .xlsx, .xls, .txt, .csv, .docx, .pdf";
		Возврат Результат;
	КонецЕсли;

	Если ПустаяСтрока(СокрЛП(ТекстФайла)) Тогда
		Результат.Сообщение = "Файл пустой или не удалось извлечь текст.";
		Возврат Результат;
	КонецЕсли;

	// Извлекаем реквизиты контрагента из текста (регулярными выражениями)
	ДанныеКонтрагента = Новый Соответствие;

	// ИНН — 10 или 12 цифр
	ПозИНН = СтрНайти(ВРег(ТекстФайла), "ИНН");
	Если ПозИНН > 0 Тогда
		Фрагмент = Сред(ТекстФайла, ПозИНН, 30);
		ЦифрыИНН = ИзвлечьЦифры(Фрагмент);
		Если СтрДлина(ЦифрыИНН) >= 10 Тогда
			ДанныеКонтрагента.Вставить("ИНН", Лев(ЦифрыИНН, ?(СтрДлина(ЦифрыИНН) >= 12, 12, 10)));
		КонецЕсли;
	КонецЕсли;

	// КПП — 9 цифр
	ПозКПП = СтрНайти(ВРег(ТекстФайла), "КПП");
	Если ПозКПП > 0 Тогда
		Фрагмент = Сред(ТекстФайла, ПозКПП, 25);
		ЦифрыКПП = ИзвлечьЦифры(Фрагмент);
		Если СтрДлина(ЦифрыКПП) >= 9 Тогда
			ДанныеКонтрагента.Вставить("КПП", Лев(ЦифрыКПП, 9));
		КонецЕсли;
	КонецЕсли;

	// ОГРН — 13 или 15 цифр
	ПозОГРН = СтрНайти(ВРег(ТекстФайла), "ОГРН");
	Если ПозОГРН > 0 Тогда
		Фрагмент = Сред(ТекстФайла, ПозОГРН, 30);
		ЦифрыОГРН = ИзвлечьЦифры(Фрагмент);
		Если СтрДлина(ЦифрыОГРН) >= 13 Тогда
			ДанныеКонтрагента.Вставить("ОГРН", Лев(ЦифрыОГРН, ?(СтрДлина(ЦифрыОГРН) >= 15, 15, 13)));
		КонецЕсли;
	КонецЕсли;

	// Наименование — ищем ООО/АО/ИП + название в кавычках
	Для Каждого ОПФ Из СтрРазделить("ООО,ОАО,ЗАО,АО,ПАО,ИП", ",") Цикл
		ПозОПФ = СтрНайти(ВРег(ТекстФайла), ОПФ);
		Если ПозОПФ > 0 Тогда
			Фрагмент = Сред(ТекстФайла, ПозОПФ, 100);
			// Ищем название в кавычках
			ПозОткр = 0;
			ПозЗакр = 0;
			Для Каждого Откр Из СтрРазделить("""«", "") Цикл
				П = СтрНайти(Фрагмент, Откр);
				Если П > 0 И (ПозОткр = 0 ИЛИ П < ПозОткр) Тогда
					ПозОткр = П;
				КонецЕсли;
			КонецЦикла;
			Если ПозОткр > 0 Тогда
				Для Каждого Закр Из СтрРазделить("""»", "") Цикл
					П = СтрНайти(Фрагмент, Закр, , ПозОткр + 1);
					Если П > 0 И (ПозЗакр = 0 ИЛИ П < ПозЗакр) Тогда
						ПозЗакр = П;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			Если ПозОткр > 0 И ПозЗакр > ПозОткр Тогда
				Название = Сред(Фрагмент, ПозОткр + 1, ПозЗакр - ПозОткр - 1);
				ДанныеКонтрагента.Вставить("Наименование", ОПФ + " """ + Название + """");
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	JSONРезультат = ЗаписьJSON.Закрыть();

	Результат.Сообщение = "Извлечённые реквизиты контрагента:" + Символы.ПС + JSONРезультат + Символы.ПС +
		"Исходный текст файла (первые 2000 символов):" + Символы.ПС + Лев(ТекстФайла, 2000) + Символы.ПС +
		"Для создания карточки используйте CreateReference с извлечёнными данными.";

	Возврат Результат;

КонецФункции

// Извлекает последовательность цифр из строки (пропуская буквы, пробелы, двоеточия)
Функция ИзвлечьЦифры(Строка_)
	Результат = "";
	НачалоЦифр = Ложь;
	Для Инд = 1 По СтрДлина(Строка_) Цикл
		Сим = Сред(Строка_, Инд, 1);
		Если СтрНайти("0123456789", Сим) > 0 Тогда
			НачалоЦифр = Истина;
			Результат = Результат + Сим;
		ИначеЕсли НачалоЦифр Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
