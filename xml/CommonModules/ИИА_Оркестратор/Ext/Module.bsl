...
Функция СобратьФактыПоследнихДействий(СсылкаДиалога) Экспорт
	...
	Для Индекс = КоличествоСообщений - 1 По 0 Цикл
		...
		ИИА_Сервер.ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "DEBUG_FACTS: Msg #" + Индекс + " Author=" + Строка(СтрокаСообщения.Автор) + " Type=" + Строка(СтрокаСообщения.ТипСообщения));
		...
		Если СтрокаСообщения.Автор = Перечисления.ИИА_АвторСообщения.Система 
			...
			ИИА_Сервер.ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "DEBUG_FACTS: Text start=" + Лев(ТекстСообщения, 50));
