...
Функция ПрименитьПолитикуВосстановления(СсылкаДиалога, КодОшибки, СообщениеОшибки) Экспорт
	...
	ИначеЕсли ЛокКод = "field_not_found" Тогда
		...
		Если НЕ ПустаяСтрока(DSLИсходный) И НЕ ПустаяСтрока(ПоляДляRetry) Тогда
			...
			Если НЕ ПустаяСтрока(ОтветИсправления.DSL) И НЕ ЭтоРанняяСдачаShowInfo(СсылкаДиалога, ОтветИсправления.DSL) Тогда
				ИИА_Сервер.СохранитьАртефактПланировщика(СсылкаДиалога, ОтветИсправления.DSL, "recovery_field_not_found", Формат(ТекущаяДатаСеанса(), "ДФ=yyyyMMddHHmmss"));
				Результат.Обработано = Истина;
				Результат.Действие = "repair_dsl";
				ПринятоИсправление = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ ПринятоИсправление Тогда
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ЛокКод = "query_syntax_error" Тогда
		АртефактПлана = ИИА_Сервер.ПолучитьАртефактПланировщика(СсылкаДиалога);
		DSLИсходный = ?(АртефактПлана <> Неопределено И АртефактПлана.Свойство("PlannedDSL"), АртефактПлана.PlannedDSL, "");
		Если НЕ ПустаяСтрока(DSLИсходный) Тогда
			...
			Если НЕ ПустаяСтрока(ОтветИсправления.DSL) Тогда
				ИИА_Сервер.СохранитьАртефактПланировщика(СсылкаДиалога, ОтветИсправления.DSL, "recovery_query_syntax", Формат(ТекущаяДатаСеанса(), "ДФ=yyyyMMddHHmmss"));
				Результат.Обработано = Истина;
				Результат.Действие = "repair_syntax";
			КонецЕсли;
		КонецЕсли;
