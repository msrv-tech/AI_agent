...
Функция СгенерироватьПродолжениеВыполнения(СсылкаДиалога) Экспорт
	...
	// Фаза 1: Обновление плана (пропускаем при пустых фактах - нет новых результатов DSL)
...
Процедура ВыполнитьФинальнуюПроверкуИSummary(СсылкаДиалога) Экспорт
	...
	Если НЕ ПустаяСтрока(ТекстSummary) Тогда
		...
		// Явное уведомление о резюме — на случай, если «НовоеСообщение» из фонового задания не дойдёт до клиента
		ОтправитьУведомление(СсылкаДиалога, "РезюмеДобавлено");
	Иначе
...
Процедура ОтправитьУведомление(СсылкаДиалога, ТипСобытия)
	...
	ИИА_Сервер.УведомитьОбОбновленииДиалога(СсылкаДиалога, ТипСобытия);
