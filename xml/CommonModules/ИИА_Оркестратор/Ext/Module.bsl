...
Функция ОбновитьПланПоРезультатам(СсылкаДиалога) Экспорт
	...
	Для Индекс = КоличествоСообщений - 1 По 0 Цикл
		...
		// Логируем для отладки (в лог диалога, не в сообщения)
		// ИИА_Сервер.ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "DEBUG: Msg #" + Индекс + " Author=" + Строка(СтрокаСообщения.Автор) + " Type=" + Строка(СтрокаСообщения.ТипСообщения));
		
		Если СтрокаСообщения.Автор = Перечисления.ИИА_АвторСообщения.Система 
			ИЛИ СтрокаСообщения.Автор = Перечисления.ИИА_АвторСообщения.ИИ Тогда
			
			ТекстСообщения = ?(СтрокаСообщения.Автор = Перечисления.ИИА_АвторСообщения.ИИ, СтрокаСообщения.ТекстКода, СтрокаСообщения.Текст);
			...
			ЭтоОшибка    = (СтрокаСообщения.ТипСообщения = Перечисления.ИИА_ТипСообщения.Ошибка);
			ЭтоDSL       = (СтрокаСообщения.Автор = Перечисления.ИИА_АвторСообщения.ИИ И НЕ ПустаяСтрока(СтрокаСообщения.ТекстКода));
