...
Процедура ВыполнитьЦикл(СсылкаДиалога, НачальныеТокены = 0) Экспорт
	...
	СчетчикОдинаковыхОшибок = 0;
	ПредыдущийКлючОшибки = "";
	МаксОдинаковыхОшибок = 3;
	
	SoftЛимитОтправлен = Ложь;
	...
	Пока Истина Цикл
		...
		Попытка
			...
			// 3.5. Остановка при N подряд одинаковых ошибках
			ТекстФактов = СобратьФактыПоследнихДействий(СсылкаДиалога);
			КлючОшибки = ПолучитьКлючПоследнейОшибкиИзФактов(ТекстФактов);
			Если НЕ ПустаяСтрока(КлючОшибки) Тогда
				Если КлючОшибки = ПредыдущийКлючОшибки Тогда
					СчетчикОдинаковыхОшибок = СчетчикОдинаковыхОшибок + 1;
				Иначе
					СчетчикОдинаковыхОшибок = 1;
					ПредыдущийКлючОшибки = КлючОшибки;
				КонецЕсли;
				Если СчетчикОдинаковыхОшибок >= МаксОдинаковыхОшибок Тогда
					ИИА_Сервер.ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[Система] Остановка: " + Строка(МаксОдинаковыхОшибок) + " подряд одинаковых ошибок.");
					ПропуститьНевыполненныеШагиПлана(СсылкаДиалога);
					ВыполнитьФинальнуюПроверкуИSummary(СсылкаДиалога);
					ИИА_Сервер.УстановитьОркестраторВключен(СсылкаДиалога, Ложь);
					Прервать;
...
Процедура ПротоколироватьМетрикуСтадии(СсылкаДиалога, StageName, Начало, Успех, Ошибка = "")
	ДлительностьМС = 0;
	Попытка
