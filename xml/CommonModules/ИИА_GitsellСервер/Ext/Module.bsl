...
Функция Gitsell_НачатьDeviceFlow(InstanceLabel = "1C Agent", UserAgent = "1C-Enterprise") Экспорт
	...
	Исключение
		...
		Если СтрНайти(ВРег(ТекстОшибки), "БЕЗОПАСН") > 0 ИЛИ СтрНайти(ВРег(ТекстОшибки), "SAFE") > 0 Тогда
			Результат.Ошибка = "Не выдано разрешение на доступ к внешнему ресурсу gitsell.ru. Проверьте разрешения расширения.";
		Иначе
...
Процедура Gitsell_СохранитьAiProxyToken(Токен, Email = "") Экспорт
	...
	НоваяЗапись = НаборЗаписей.Количество() = 0;
	...
	Запись.Модель = МодельПоУмолчанию();
...
// Для Фреш сам по себе безопасный режим не является ошибкой:
// доступ к HTTP определяется выданными разрешениями на внешние ресурсы.
//
...
Функция Gitsell_ПроверитьModels() Экспорт
	...
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		Если СтрНайти(ВРег(ТекстОшибки), "БЕЗОПАСН") > 0 ИЛИ СтрНайти(ВРег(ТекстОшибки), "SAFE") > 0 Тогда
			Результат.Ошибка = "Не выдано разрешение на доступ к внешнему ресурсу. Проверьте разрешения расширения.";
		Иначе
			Результат.Ошибка = "Ошибка соединения: " + ТекстОшибки;
		КонецЕсли;
	КонецПопытки;
...
Функция МодельПоУмолчанию() Экспорт
	Возврат "gpt-5.6-luna";
КонецФункции
...
Функция Gitsell_СписокМоделей() Экспорт
	...
	Модели.Добавить(Новый Структура("id, name", "gpt-5.6-luna", "openai/gpt-5.6-luna"));
	...
	Модели.Добавить(Новый Структура("id, name", "GigaChat-3-Ultra", "gigachat/GigaChat-3-Ultra"));
	
	// YandexGPT
	Модели.Добавить(Новый Структура("id, name", "yandexgpt", "yandex/YandexGPT Pro"));
