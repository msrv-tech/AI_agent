...
Функция ПолучитьПромптГенерацииСледующегоШага(ПланJSON, РезультатПоследнегоDSL, ДоступныеПоля = "", НеудачныеОбъекты = "") Экспорт
	...
	"- CheckObjectExists: {""action"":""CheckObjectExists"",""object_type"":""РегистрНакопления"",""object_name"":""ЗапасыНаСкладах""}. object_type=Справочник/Документ/РегистрНакопления/РегистрСведений." + Символы.ПС +
	"- GetObjectFields: {""action"":""GetObjectFields"",""object_type"":""РегистрНакопления"",""object_name"":""ЗапасыНаСкладах""}. object_type=Справочник/Документ/РегистрНакопления/РегистрСведений." + Символы.ПС +
	...
	"- RunQuery: {""action"":""RunQuery"",""query"":""ВЫБРАТЬ ... ГДЕ Период МЕЖДУ ДАТАВРЕМЯ(2025,1,1,0,0,0) И ДАТАВРЕМЯ(2025,1,31,23,59,59)""}. ВСЕГДА используй ДАТАВРЕМЯ в запросе для дат. НЕ передавай даты через parameters." + Символы.ПС +
...
"- ВАЖНО ПО ДАТАМ: Для дат в RunQuery ВСЕГДА используй ДАТАВРЕМЯ(год,месяц,день,час,минута,сек) прямо в запросе. НЕ передавай даты через parameters — parameters только для ссылок, текста и др. Пример: ГДЕ Период МЕЖДУ ДАТАВРЕМЯ(2025,1,1,0,0,0) И ДАТАВРЕМЯ(2025,1,31,23,59,59)." + Символы.ПС +
	...
	"- RunQuery{query,parameters?}; ShowInfo{message}; GetMetadata{metadata_type?}; GetObjectFields{object_type,object_name}; CheckObjectExists{object_type,object_name} — Справочник, Документ, РегистрНакопления, РегистрСведений;" + Символы.ПС +
