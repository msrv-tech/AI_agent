...
// Формирует текстовый блок с контекстом RAG для промпта
//
// Параметры:
...
Функция СформироватьКонтекстRAG(Знач ТекстЗапроса) Экспорт
	...
	Сущности = ИИА_Сервер.ИзвлечьСущностиДляRAG(ТекстЗапроса);
	...
	РезультатыПоиска = ИИА_RAG_Поиск.ВыполнитьПоиск(ТекстДляПоиска, 5);
...
// Формирует промпт для экстрактора сущностей (выделение названий метаданных)
//
// Параметры:
//  ТекстЗапроса - Строка - исходный текст пользователя
//
// Возвращаемое значение:
//  Строка - текст промпта
//
Функция ПолучитьПромптЭкстрактораСущностей(ТекстЗапроса) Экспорт
	...
	Промпт = "Ты — эксперт по метаданным 1С. Твоя задача — выделить из фразы пользователя только названия объектов (документы, справочники, регистры).
	|Правила:
	|1. Верни только названия сущностей через запятую.
	|2. Используй именительный падеж (например, 'Расходная накладная', а не 'расходных накладных').
	|3. Если сущностей нет, верни слово 'НЕТ'.
	|4. Игнорируй глаголы (найти, покажи), даты (апрель, 2022) и количества (сколько).
	|5. Не пиши ничего, кроме названий объектов.
	|
