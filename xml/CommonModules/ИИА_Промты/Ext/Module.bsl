...
Функция ПолучитьПромптСозданияПлана(ТекстЗадачи) Экспорт
	...
	Промпт = "Составь краткий план выполнения задачи (3-6 пунктов)." + Символы.ПС +
...
Функция ПолучитьТекстПромпта_Агент()
	...
	"Формат ответа: ТОЛЬКО JSON объект {""dsl_version"":1,""steps"":[{...}]}. Никакого текста вне JSON." + Символы.ПС +
