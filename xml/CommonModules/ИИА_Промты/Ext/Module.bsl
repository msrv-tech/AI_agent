...
Функция ПолучитьТекстПромптаПланировщика() Экспорт
	...
	"Для задач по ценам, продажам, задолженности — используй шаблоны из ДОМЕННЫХ ЗНАНИЙ ERP 2. Не начинай с GetMetadata/CheckObjectExists если объект известен из домена." + Символы.ПС +
	КонтрактОтвета;
...
Функция ПолучитьПромптГенерацииСледующегоШага(ПланJSON, РезультатПоследнегоDSL, ДоступныеПоля = "", НеудачныеОбъекты = "") Экспорт
	...
	Промпт = Промпт +
	"DOMAIN SHORTCUTS: Для цен → РегистрСведений.ЦеныНоменклатуры.СрезПоследних. Для задолженности → РегистрНакопления.РасчетыСКлиентами.Остатки. Для продаж → Документ.РеализацияТоваровУслуг + ТЧ Товары." + Символы.ПС;
...
"- ВАЖНО ПО ДАТАМ: Для дат в RunQuery ВСЕГДА используй ДАТАВРЕМЯ(год,месяц,день,час,минута,сек) прямо в запросе. НЕ передавай даты через parameters — parameters только для ссылок, текста и др. Пример: ГДЕ Период МЕЖДУ ДАТАВРЕМЯ(2025,1,1,0,0,0) И ДАТАВРЕМЯ(2025,1,31,23,59,59)." + Символы.ПС +
	...
	"" + Символы.ПС +
	"ДОМЕННЫЕ ЗНАНИЯ ERP 2 (1С:ERP Управление предприятием):" + Символы.ПС +
	"" + Символы.ПС +
	"=== СЦЕНАРИЙ 1: ЦЕНЫ ===" + Символы.ПС +
	"Регистр цен: РегистрСведений.ЦеныНоменклатуры (или ЦеныНоменклатуры25 в новых версиях)." + Символы.ПС +
	"Ключевые поля: Номенклатура, ВидЦены, Цена, Валюта, Период." + Символы.ПС +
	"Шаблон запроса текущей цены:" + Символы.ПС +
	"  ВЫБРАТЬ Цена, ВидЦены.Наименование КАК ВидЦены, Валюта ИЗ РегистрСведений.ЦеныНоменклатуры.СрезПоследних КАК Цены ГДЕ Номенклатура.Наименование ПОДОБНО ""%<товар>%""" + Символы.ПС +
	"Виды цен (справочник ВидыЦен): Оптовая, Розничная, Закупочная и др." + Символы.ПС +
	"Для сравнения с ценой производителя — запрашивай оба ВидЦены." + Символы.ПС +
	"" + Символы.ПС +
	"=== СЦЕНАРИЙ 2: ИСТОРИЯ ПРОДАЖ + МАРЖИНАЛЬНОСТЬ ===" + Символы.ПС +
	"Документ продаж: Документ.РеализацияТоваровУслуг." + Символы.ПС +
	"Шапка: Дата, Партнер, Контрагент, Организация, СуммаДокумента." + Символы.ПС +
	"Табличная часть Товары: Номенклатура, Количество, Цена, Сумма, СуммаНДС." + Символы.ПС +
	"ВАЖНО (ERP 2): В шапке есть И Партнер, И Контрагент. Партнер — бизнес-субъект, Контрагент — юрлицо." + Символы.ПС +
	"Для маржинальности сравнивай цену продажи с закупочной:" + Символы.ПС +
	"  - Цена продажи: РеализацияТоваровУслуг.Товары.Цена" + Символы.ПС +
	"  - Себестоимость: ЦеныНоменклатуры с ВидЦены=""Закупочная"" (или регистр СебестоимостьТоваров)" + Символы.ПС +
	"  - Маржа = (ЦенаПродажи - Себестоимость) / ЦенаПродажи * 100" + Символы.ПС +
	"Шаблон запроса:" + Символы.ПС +
	"  ВЫБРАТЬ Реализация.Дата, Товары.Номенклатура.Наименование КАК Товар, Товары.Количество, Товары.Цена, Товары.Сумма КАК СуммаПродажи ИЗ Документ.РеализацияТоваровУслуг КАК Реализация ЛЕВОЕ СОЕДИНЕНИЕ Реализация.Товары КАК Товары ГДЕ Реализация.Дата МЕЖДУ ДАТАВРЕМЯ(...) И ДАТАВРЕМЯ(...) И Реализация.Партнер.Наименование ПОДОБНО ""%<клиент>%""" + Символы.ПС +
	"" + Символы.ПС +
	"=== СЦЕНАРИЙ 3: ДЕБИТОРСКАЯ ЗАДОЛЖЕННОСТЬ ===" + Символы.ПС +
	"Регистр: РегистрНакопления.РасчетыСКлиентами." + Символы.ПС +
	"Виртуальная таблица остатков: РасчетыСКлиентами.Остатки." + Символы.ПС +
	"Шаблон текущего долга:" + Символы.ПС +
	"  ВЫБРАТЬ Партнер.Наименование КАК Клиент, СУММА(СуммаОстаток) КАК Долг ИЗ РегистрНакопления.РасчетыСКлиентами.Остатки КАК Расчеты СГРУППИРОВАТЬ ПО Партнер" + Символы.ПС +
	"Для просрочки: сравнивай СрокОплаты с текущей датой через ДАТАВРЕМЯ." + Символы.ПС +
	"ВАЖНО (ERP 2): В РасчетыСКлиентами ключ — Партнер (не Контрагент)." + Символы.ПС +
	"" + Символы.ПС +
	"=== СВОДКА ПО КЛИЕНТУ (мульти-источник) ===" + Символы.ПС +
	"При запросе о клиенте объединяй данные:" + Символы.ПС +
	"1. FindReferenceByName → найди клиента (ищи в Партнеры, не в Контрагенты)" + Символы.ПС +
	"2. RunQuery → РасчетыСКлиентами.Остатки (текущий долг)" + Символы.ПС +
	"3. RunQuery → РеализацияТоваровУслуг (история продаж)" + Символы.ПС +
	"4. RunQuery → ЦеныНоменклатуры (текущие цены)" + Символы.ПС +
	"Покажи консолидированный результат в одном ShowInfo." + Символы.ПС +
	"" + Символы.ПС +
	"=== БОНУС: ЗАГРУЗКА ПРАЙС-ЛИСТА ИЗ EXCEL ===" + Символы.ПС +
	"Для загрузки прайс-листа используй ImportPriceList{file_path:""путь"", price_type:""Оптовая""}." + Символы.ПС +
	"Файл должен быть в формате Excel (.xlsx) с колонками: Наименование | Цена | Валюта." + Символы.ПС +
	"" + Символы.ПС +
	"=== БОНУС: СОЗДАНИЕ КОНТРАГЕНТА ИЗ ФАЙЛА ===" + Символы.ПС +
	"Для создания карточки контрагента из файла (xlsx, pdf, docx, txt):" + Символы.ПС +
	"1. ParseFileForContractor{file_path:""путь к файлу""} → извлечёт реквизиты" + Символы.ПС +
	"2. CreateReference{object:""Catalog.Партнеры"", data:{Наименование:""..."", ИНН:""..."", ...}} → создаст карточку" + Символы.ПС +
	"Поддерживаемые форматы: .xlsx, .xls, .pdf, .docx, .txt";
