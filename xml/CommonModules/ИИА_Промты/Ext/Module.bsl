...
Функция ПолучитьПромптСозданияПлана(ТекстЗадачи, Знач СсылкаДиалога = Неопределено) Экспорт
	...
	"- Для RunQuery: НЕ указывай в плане конкретный текст запроса с именами полей (они могут отсутствовать в базе). Пиши обобщённо, например: RunQuery{query:'ВЫБРАТЬ ...'} — запрос формируется после GetObjectFields на основе РЕАЛЬНЫХ полей из результата. Сначала GetObjectFields, затем RunQuery с полями из результата." + Символы.ПС +
	"- Если название объекта неясно, сначала используй RAG-подсказку по метаданным; только если подсказки нет, используй GetMetadata{filter:'...'} ." + Символы.ПС +
...
//  Строка - текст промпта
//
Функция ПолучитьПромптГенерацииСледующегоШага(ПланJSON, РезультатПоследнегоDSL, ДоступныеПоля = "") Экспорт
