...
//  БезRAG - Булево - (опционально) если Истина, RAG-контекст не будет добавляться (защита от рекурсии)
//
// Возвращаемое значение:
//  Структура - структура с сообщениями для ИИ
//
Функция СформироватьПромпт(ТипДиалога, ТипСообщения, ТекстПользователя, История, СистемныйПромпт = "", Знач СсылкаДиалога = Неопределено, Знач БезRAG = Ложь) Экспорт
	...
	Если НЕ БезRAG Тогда
		ТекстRAG = СформироватьКонтекстRAG(ТекстПользователя, СсылкаДиалога);
...
Функция ПолучитьТекстПромптаПланировщика() Экспорт
	...
	"Если в запросе упоминаются объекты метаданных, обязательно начни с проверки их существования или получения полей." + Символы.ПС +
...
Функция ПолучитьПромптОбновленияИПланирования(ПланJSON, РезультатПоследнегоDSL) Экспорт
	...
	"- ЗАПРЕЩЕНО любые UI/браузерные действия и выдуманные action: click, find_element, open_form, ui_*, navigate и т.п." + Символы.ПС +
...
//  Строка - текст промпта
//
Функция ПолучитьПромптИсправленияDSL(ТекстОшибки, ИсходныйDSL) Экспорт
	...
	"Исправь ошибку и верни новый DSL-сценарий." + Символы.ПС +
	"Если ошибка в имени поля или объекта, используй GetMetadata или GetObjectFields для уточнения." + Символы.ПС +
	"Ответ: только валидный JSON объект DSL.";
