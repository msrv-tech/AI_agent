...
Функция ПолучитьТекстПромптаПланировщика() Экспорт
	...
	"Для задач по ценам, продажам, задолженности — используй объекты из ДОМЕННЫХ ЗНАНИЙ (они сгенерированы из реальных метаданных). Не начинай с GetMetadata/CheckObjectExists если объект известен из домена." + Символы.ПС +
	КонтрактОтвета;
...
Функция ПолучитьПромптГенерацииСледующегоШага(ПланJSON, РезультатПоследнегоDSL, ДоступныеПоля = "", НеудачныеОбъекты = "") Экспорт
	...
	Промпт = Промпт +
	"DOMAIN SHORTCUTS: " + СформироватьДоменныеШорткаты() + Символы.ПС;
...
"- ВАЖНО ПО ДАТАМ: Для дат в RunQuery ВСЕГДА используй ДАТАВРЕМЯ(год,месяц,день,час,минута,сек) прямо в запросе. НЕ передавай даты через parameters — parameters только для ссылок, текста и др. Пример: ГДЕ Период МЕЖДУ ДАТАВРЕМЯ(2025,1,1,0,0,0) И ДАТАВРЕМЯ(2025,1,31,23,59,59)." + Символы.ПС +
	...
	СхемыДействий + Символы.ПС +
	"guid в FindReferenceByGUID - только строка UUID (например ""a1b2c3d4-e5f6-7890-abcd-ef1234567890"")." + Символы.ПС +
	"" + Символы.ПС +
	СформироватьДоменныйПромпт();
...
// Динамическая генерация доменного промпта на основе реальных метаданных конфигурации
Функция СформироватьДоменныйПромпт()

	Части = Новый Массив;
	Части.Добавить("ДОМЕННЫЕ ЗНАНИЯ (сгенерировано динамически из метаданных конфигурации):");
	Части.Добавить("");

	// === СЦЕНАРИЙ 1: ЦЕНЫ ===
	РегистрЦен = НайтиРегистрЦен();
	Если НЕ ПустаяСтрока(РегистрЦен.ПолноеИмя) Тогда
		Части.Добавить("=== СЦЕНАРИЙ 1: ЦЕНЫ ===");
		Части.Добавить("Регистр цен: " + РегистрЦен.ПолноеИмя + " (" + РегистрЦен.Синоним + ").");
		Части.Добавить("Поля: " + РегистрЦен.Поля);
		Части.Добавить("Для текущих цен используй виртуальную таблицу .СрезПоследних.");
		Части.Добавить("Шаблон: ВЫБРАТЬ <поля> ИЗ " + РегистрЦен.ПолноеИмя + ".СрезПоследних ГДЕ Номенклатура.Наименование ПОДОБНО ""%<товар>%""");

		СправочникВидовЦен = НайтиОбъектМетаданных("Catalog", "ВидыЦен,ВидЦен,ТипыЦен");
		Если НЕ ПустаяСтрока(СправочникВидовЦен) Тогда
			Части.Добавить("Виды цен хранятся в справочнике " + СправочникВидовЦен + ". Для сравнения с закупочной ценой — запрашивай оба ВидЦены.");
		КонецЕсли;
		Части.Добавить("");
	КонецЕсли;

	// === СЦЕНАРИЙ 2: ИСТОРИЯ ПРОДАЖ + МАРЖИНАЛЬНОСТЬ ===
	ДокПродаж = НайтиДокументПродаж();
	Если НЕ ПустаяСтрока(ДокПродаж.ПолноеИмя) Тогда
		Части.Добавить("=== СЦЕНАРИЙ 2: ИСТОРИЯ ПРОДАЖ + МАРЖИНАЛЬНОСТЬ ===");
		Части.Добавить("Документ продаж: " + ДокПродаж.ПолноеИмя + " (" + ДокПродаж.Синоним + ").");
		Части.Добавить("Реквизиты шапки: " + ДокПродаж.РеквизитыШапки);
		Если НЕ ПустаяСтрока(ДокПродаж.ТабличныеЧасти) Тогда
			Части.Добавить("Табличные части: " + ДокПродаж.ТабличныеЧасти);
		КонецЕсли;

		// Проверяем наличие Партнер и Контрагент
		Если СтрНайти(ДокПродаж.РеквизитыШапки, "Партнер") > 0
			И СтрНайти(ДокПродаж.РеквизитыШапки, "Контрагент") > 0 Тогда
			Части.Добавить("ВАЖНО: В шапке есть И Партнер, И Контрагент. Партнер — бизнес-субъект, Контрагент — юрлицо. Фильтруй по Партнер.");
		КонецЕсли;

		Части.Добавить("Для маржинальности: Маржа = (ЦенаПродажи - Себестоимость) / ЦенаПродажи * 100.");
		Если НЕ ПустаяСтрока(РегистрЦен.ПолноеИмя) Тогда
			Части.Добавить("Себестоимость бери из " + РегистрЦен.ПолноеИмя + " с ВидЦены=""Закупочная"" или из регистра СебестоимостьТоваров.");
		КонецЕсли;
		Части.Добавить("");
	КонецЕсли;

	// === СЦЕНАРИЙ 3: ДЕБИТОРСКАЯ ЗАДОЛЖЕННОСТЬ ===
	РегистрРасчетов = НайтиРегистрРасчетов();
	Если НЕ ПустаяСтрока(РегистрРасчетов.ПолноеИмя) Тогда
		Части.Добавить("=== СЦЕНАРИЙ 3: ДЕБИТОРСКАЯ ЗАДОЛЖЕННОСТЬ ===");
		Части.Добавить("Регистр: " + РегистрРасчетов.ПолноеИмя + " (" + РегистрРасчетов.Синоним + ").");
		Части.Добавить("Поля: " + РегистрРасчетов.Поля);
		Части.Добавить("Виртуальная таблица остатков: " + РегистрРасчетов.Имя + ".Остатки.");

		// Определяем ключ клиента
		КлючКлиента = "Контрагент";
		Если СтрНайти(РегистрРасчетов.Поля, "Партнер") > 0 Тогда
			КлючКлиента = "Партнер";
		КонецЕсли;
		Части.Добавить("Ключ клиента в этом регистре: " + КлючКлиента + ".");
		Части.Добавить("Для просрочки: если есть поле СрокОплаты — сравнивай с текущей датой через ДАТАВРЕМЯ.");
		Части.Добавить("");
	КонецЕсли;

	// === СВОДКА ПО КЛИЕНТУ ===
	Если НЕ ПустаяСтрока(РегистрЦен.ПолноеИмя)
		И НЕ ПустаяСтрока(ДокПродаж.ПолноеИмя)
		И НЕ ПустаяСтрока(РегистрРасчетов.ПолноеИмя) Тогда

		// Определяем справочник клиентов
		СправочникКлиентов = НайтиОбъектМетаданных("Catalog", "Партнеры,Контрагенты,Клиенты");
		Если ПустаяСтрока(СправочникКлиентов) Тогда
			СправочникКлиентов = "Партнеры или Контрагенты";
		КонецЕсли;

		Части.Добавить("=== СВОДКА ПО КЛИЕНТУ (мульти-источник) ===");
		Части.Добавить("При запросе о клиенте объединяй данные:");
		Части.Добавить("1. FindReferenceByName → найди клиента (ищи в " + СправочникКлиентов + ")");
		Части.Добавить("2. RunQuery → " + РегистрРасчетов.Имя + ".Остатки (текущий долг)");
		Части.Добавить("3. RunQuery → " + ДокПродаж.ПолноеИмя + " (история продаж)");
		Части.Добавить("4. RunQuery → " + РегистрЦен.ПолноеИмя + ".СрезПоследних (текущие цены)");
		Части.Добавить("Покажи консолидированный результат в одном ShowInfo.");
		Части.Добавить("");
	КонецЕсли;

	// === БОНУСЫ (статические — DSL-действия не зависят от метаданных) ===
	Части.Добавить("=== БОНУС: ЗАГРУЗКА ПРАЙС-ЛИСТА ИЗ EXCEL ===");
	Части.Добавить("Для загрузки прайс-листа используй ImportPriceList{file_path:""путь"", price_type:""Оптовая""}.");
	Части.Добавить("Файл должен быть в формате Excel (.xlsx) с колонками: Наименование | Цена | Валюта.");
	Части.Добавить("");
	Части.Добавить("=== БОНУС: СОЗДАНИЕ КОНТРАГЕНТА ИЗ ФАЙЛА ===");
	Части.Добавить("Для создания карточки контрагента из файла (xlsx, pdf, docx, txt):");
	Части.Добавить("1. ParseFileForContractor{file_path:""путь к файлу""} → извлечёт реквизиты");
	Части.Добавить("2. CreateReference{object:""Catalog.Партнеры"", data:{Наименование:""..."", ИНН:""..."", ...}} → создаст карточку");
	Части.Добавить("Поддерживаемые форматы: .xlsx, .xls, .pdf, .docx, .txt");

	Возврат СтрСоединить(Части, Символы.ПС);

КонецФункции

// Формирует компактные доменные шорткаты из метаданных
Функция СформироватьДоменныеШорткаты()

	Части = Новый Массив;

	РегистрЦен = НайтиРегистрЦен();
	Если НЕ ПустаяСтрока(РегистрЦен.ПолноеИмя) Тогда
		Части.Добавить("Для цен → " + РегистрЦен.ПолноеИмя + ".СрезПоследних");
	КонецЕсли;

	РегистрРасчетов = НайтиРегистрРасчетов();
	Если НЕ ПустаяСтрока(РегистрРасчетов.ПолноеИмя) Тогда
		Части.Добавить("Для задолженности → " + РегистрРасчетов.ПолноеИмя + ".Остатки");
	КонецЕсли;

	ДокПродаж = НайтиДокументПродаж();
	Если НЕ ПустаяСтрока(ДокПродаж.ПолноеИмя) Тогда
		ТекстПродаж = "Для продаж → " + ДокПродаж.ПолноеИмя;
		Если НЕ ПустаяСтрока(ДокПродаж.ТабличныеЧасти) Тогда
			ТекстПродаж = ТекстПродаж + " + ТЧ " + ДокПродаж.ТабличныеЧасти;
		КонецЕсли;
		Части.Добавить(ТекстПродаж);
	КонецЕсли;

	Если Части.Количество() = 0 Тогда
		Возврат "Доменные объекты не найдены — используй GetMetadata для discovery.";
	КонецЕсли;
...
// Поиск регистра цен номенклатуры (РегистрСведений)
// Пробует варианты: ЦеныНоменклатуры, ЦеныНоменклатуры25, ЦеныТоваров
Функция НайтиРегистрЦен()

	Результат = Новый Структура("ПолноеИмя, Имя, Синоним, Поля", "", "", "", "");

	Кандидаты = СтрРазделить("ЦеныНоменклатуры,ЦеныНоменклатуры25,ЦеныТоваров", ",", Ложь);

	Для Каждого Кандидат Из Кандидаты Цикл
		Попытка
			Данные = ИИА_Метаданные.ПолучитьРеквизитыРегистраСведений(Кандидат);
			Если Данные.Найден Тогда
				Результат.ПолноеИмя = "РегистрСведений." + Данные.Имя;
				Результат.Имя = Данные.Имя;
				Результат.Синоним = Данные.Синоним;
				Результат.Поля = СформироватьСписокПолей(Данные.Реквизиты);
				Возврат Результат;
			КонецЕсли;
		Исключение
		КонецПопытки;
...
// Поиск документа продаж
// Пробует: РеализацияТоваровУслуг, РасходнаяНакладная, ПродажаТоваров
Функция НайтиДокументПродаж()

	Результат = Новый Структура("ПолноеИмя, Синоним, РеквизитыШапки, ТабличныеЧасти", "", "", "", "");

	Кандидаты = СтрРазделить("РеализацияТоваровУслуг,РасходнаяНакладная,ПродажаТоваров", ",", Ложь);

	Для Каждого Кандидат Из Кандидаты Цикл
		Попытка
			Данные = ИИА_Метаданные.ПолучитьРеквизитыДокумента(Кандидат);
			Если Данные.Найден Тогда
				Результат.ПолноеИмя = "Документ." + Данные.Имя;
				Результат.Синоним = Данные.Синоним;
				Результат.РеквизитыШапки = СформироватьСписокПолей(Данные.Реквизиты);

				// Табличные части
				ИменаТЧ = Новый Массив;
				Для Каждого ТЧ Из Данные.ТабличныеЧасти Цикл
					ИменаТЧ.Добавить(ТЧ.Имя);
				КонецЦикла;
				Результат.ТабличныеЧасти = СтрСоединить(ИменаТЧ, ", ");
...
// Поиск регистра расчетов с клиентами (РегистрНакопления)
// Пробует: РасчетыСКлиентами, ВзаиморасчетыСКонтрагентами, РасчетыСПокупателями
Функция НайтиРегистрРасчетов()

	Результат = Новый Структура("ПолноеИмя, Имя, Синоним, Поля", "", "", "", "");

	Кандидаты = СтрРазделить("РасчетыСКлиентами,РасчетыСКлиентамиПоДокументам,ВзаиморасчетыСКонтрагентами,РасчетыСПокупателями", ",", Ложь);

	Для Каждого Кандидат Из Кандидаты Цикл
		Попытка
			Данные = ИИА_Метаданные.ПолучитьРеквизитыРегистраНакопления(Кандидат);
			Если Данные.Найден Тогда
				Результат.ПолноеИмя = "РегистрНакопления." + Данные.Имя;
				Результат.Имя = Данные.Имя;
				Результат.Синоним = Данные.Синоним;
				Результат.Поля = СформироватьСписокПолей(Данные.Реквизиты);
				Возврат Результат;
...
// Универсальный поиск объекта метаданных по списку кандидатов
// ТипОбъекта: "Catalog", "Document", "InfoReg", "AccumReg"
// Кандидаты: строка с именами через запятую
Функция НайтиОбъектМетаданных(ТипОбъекта, Кандидаты)

	МассивКандидатов = СтрРазделить(Кандидаты, ",", Ложь);

	Для Каждого Кандидат Из МассивКандидатов Цикл
		Попытка
			Если ТипОбъекта = "Catalog" Тогда
				Данные = ИИА_Метаданные.ПолучитьРеквизитыСправочника(СокрЛП(Кандидат));
			ИначеЕсли ТипОбъекта = "Document" Тогда
				Данные = ИИА_Метаданные.ПолучитьРеквизитыДокумента(СокрЛП(Кандидат));
			ИначеЕсли ТипОбъекта = "InfoReg" Тогда
				Данные = ИИА_Метаданные.ПолучитьРеквизитыРегистраСведений(СокрЛП(Кандидат));
			ИначеЕсли ТипОбъекта = "AccumReg" Тогда
				Данные = ИИА_Метаданные.ПолучитьРеквизитыРегистраНакопления(СокрЛП(Кандидат));
			Иначе
				Продолжить;
			КонецЕсли;
...
// Формирует компактный список полей из массива реквизитов
Функция СформироватьСписокПолей(МассивРеквизитов)
	...
	Для Каждого Реквизит Из МассивРеквизитов Цикл
		...
		Если НЕ ПустаяСтрока(Синоним) И Синоним <> Имя Тогда
			Имена.Добавить(Имя + "(" + Синоним + ")");
		Иначе
			Имена.Добавить(Имя);
		КонецЕсли;
	КонецЦикла;
