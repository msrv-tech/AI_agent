...
Функция ПолучитьПромптГенерацииСледующегоШага(ПланJSON, РезультатПоследнегоDSL, ДоступныеПоля = "", НеудачныеОбъекты = "") Экспорт
	...
	"- GetObjectFields: {""action"":""GetObjectFields"",""object_type"":""Справочник"",""object_name"":""Контрагенты""}. Для регистров: object_type=РегистрНакопления или РегистрСведений, object_name=ЗапасыНаСкладах (без префикса)." + Символы.ПС +
	"- FindReferenceByGUID: guid — строка UUID (""xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx""), НЕ текст результата" + Символы.ПС +
	"- CreateDocument: {""action"":""CreateDocument"",""object_name"":""ПриходТовараМП"",""data"":{""Дата"":""#(ТекущаяДата)"",""Контрагент"":""#(FindReferenceByName)""}}. Для документов с датой обязательно передавай Дата в data (или используй ТекущаяДата())." + Символы.ПС +
	"- RunQuery: {""action"":""RunQuery"",""query"":""ВЫБРАТЬ ..."",""parameters"":{""ДатаНачала"":""20250101"",""ДатаОкончания"":""20250131""}}. Если в запросе есть параметры (&ИмяПараметра), передавай их в поле parameters. Для «последний месяц» вычисли даты и подставь в parameters." + Символы.ПС +
...
Функция ПолучитьТекстПромпта_Агент()
	...
	"- ДОБАВИТЬКДАТЕ: порядок аргументов (Дата, ЕдиницаИзмерения, Количество). ПРАВИЛЬНО: ДОБАВИТЬКДАТЕ(ТекущаяДата(), ДЕНЬ, -7). НЕПРАВИЛЬНО: ДОБАВИТЬКДАТЕ(Дата, -7, ДЕНЬ). Для периода: НАЧАЛОПЕРИОДА(Дата, МЕСЯЦ), КОНЕЦПЕРИОДА(Дата, МЕСЯЦ), МЕЖДУ." + Символы.ПС +
...
Функция ПолучитьТекстПромпта_Запрос1С()
	...
	"Для дат: ДОБАВИТЬКДАТЕ(Дата, ДЕНЬ, -7) — порядок: Дата, ЕдиницаИзмерения, Количество. НЕПРАВИЛЬНО: ДОБАВИТЬКДАТЕ(Дата, -7, ДЕНЬ)." + Символы.ПС +
	"Если в RunQuery есть параметры (&ИмяПараметра), передавай их в поле parameters: parameters: {ДатаНачала: '20250101', ДатаОкончания: '20250131'}." + Символы.ПС +
