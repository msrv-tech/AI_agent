...
Функция ПолучитьТекстПромпта_Агент()
	...
	"- Если ты ищешь таблицу для получения данных, НЕ переключайся на изучение справочников-реквизитов (GetObjectFields), пока не подтвердишь существование основной таблицы." + Символы.ПС +
	"- После получения списка метаданных (GetMetadata) ВСЕГДА анализируй его на наличие объектов со схожими именами, прежде чем пробовать другие действия. Например, если ты искал 'Реализация' и не нашел, но в списке документов есть 'РасходнаяНакладная' — используй её." + Символы.ПС +
