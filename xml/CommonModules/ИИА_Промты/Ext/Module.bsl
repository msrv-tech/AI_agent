...
Функция СформироватьПромпт(ТипДиалога, ТипСообщения, ТекстПользователя, История, СистемныйПромпт = "") Экспорт
	...
	//Если ТипДиалога = Перечисления.ИИА_ТипДиалога.Агент Тогда
		...
		ТекстRAG = СформироватьКонтекстRAG(ТекстПользователя);
	//КонецЕсли;
...
Функция СформироватьStateSummary(История)
	...
	// ОЧИСТКА ПАМЯТИ: Если последний результат системы содержит успешный Discovery, 
	// помечаем старые ошибки как неактуальные
	Если СтрНайти(ПоследнийСистемныйРезультат, "Получены метаданные") > 0 Тогда
		ПоследнийСистемныйРезультат = ПоследнийСистемныйРезультат + Символы.ПС + 
		"ИНФО: База исследована. Забудь про предыдущие ошибки 'объект не найден' и используй новые данные.";
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(ПоследнийСистемныйРезультат) Тогда
		// СЖАТИЕ КОНТЕКСТА: Ограничиваем длину результата системы в StateSummary
		State = State + "Последний результат системы:" + Символы.ПС + ОбрезатьТекстДляState(ПоследнийСистемныйРезультат, 600) + Символы.ПС;
	КонецЕсли;
	
	// СПИСОК ОШИБОК И ПОПЫТОК (для предотвращения зацикливания)
	МассивОшибок = Новый Массив;
	Для Индекс = Количество - 1 По 0 Цикл
		СтрокаИстории = История[Индекс];
		Если СтрокаИстории.ТипСообщения = Перечисления.ИИА_ТипСообщения.Ошибка Тогда
			МассивОшибок.Добавить(ОбрезатьТекстДляState(СтрокаИстории.Текст, 200));
		КонецЕсли;
		Если МассивОшибок.Количество() >= 5 Тогда Прервать; КонецЕсли;
	КонецЦикла;
	
	Если МассивОшибок.Количество() > 0 Тогда
		State = State + "Предыдущие ошибки (НЕ ПОВТОРЯЙ ИХ):" + Символы.ПС;
		Для Каждого Ошибка Из МассивОшибок Цикл
			State = State + "! " + Ошибка + Символы.ПС;
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьПромптСозданияПлана(ТекстЗадачи) Экспорт
	...
	ТекстRAG = СформироватьКонтекстRAG(ТекстЗадачи);
	
	Промпт = "";
	Если НЕ ПустаяСтрока(ТекстRAG) Тогда
		Промпт = ТекстRAG + Символы.ПС;
	КонецЕсли;
	
	Промпт = Промпт + "Составь краткий план выполнения задачи (3-6 пунктов) в терминах DSL-действий агента 1С." + Символы.ПС +
...
Функция ПолучитьПромптОбновленияИПланирования(ПланJSON, РезультатПоследнегоDSL) Экспорт
	...
	// Для генерации следующего шага тоже полезно подмешать RAG на основе невыполненных пунктов
	ТекстRAG = "";
	
	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(ПланJSON);
	Попытка
		ПланМассив = ПрочитатьJSON(ЧтениеJSON);
	Исключение
		ПланМассив = Неопределено;
	КонецПопытки;
	ЧтениеJSON.Закрыть();
	
	Если ТипЗнч(ПланМассив) = Тип("Массив") Тогда
		Для Каждого Пункт Из ПланМассив Цикл
			Если НЕ Пункт.Выполнена Тогда
				ТекстRAG = СформироватьКонтекстRAG(Пункт.Задача);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Промпт = Промпт + "Сделай ДВА действия в одном ответе:" + Символы.ПС +
...
// Формирует текстовый блок с контекстом метаданных на основе RAG
//
// Параметры:
//  ТекстЗапроса - Строка - поисковый запрос (задача пользователя или пункт плана)
//
// Возвращаемое значение:
//  Строка - текстовый блок для промпта
Функция СформироватьКонтекстRAG(Знач ТекстЗапроса) Экспорт
	...
	РезультатыПоиска = ИИА_RAG_Поиск.ВыполнитьПоиск(ТекстЗапроса, 5);
	...
	"На основе твоего запроса в базе найдены следующие релевантные объекты:" + Символы.ПС;
	
	Для Каждого Результат Из РезультатыПоиска Цикл
		
		ОписаниеОбъекта = СтрШаблон("- %1.%2 (%3)", Результат.Тип, Результат.Имя, Результат.Синоним);
...
Функция ПолучитьТекстПромпта_Агент()
	...
	"- ТЕБЕ ДОСТУПЕН RAG-ПОИСК: В сообщениях системы может быть блок 'ПОДСКАЗКА ПО МЕТАДАННЫМ (RAG)'. Всегда отдавай приоритет именам объектов и полей из этого блока." + Символы.ПС +
