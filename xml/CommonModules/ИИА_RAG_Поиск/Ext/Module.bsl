...
Функция ВыполнитьПоиск(Знач ЗапросТекст, Знач TopK = 10, Знач Контекст = Неопределено) Экспорт
	...
	Для каждого СтрДетали Из ДеталиЧанков Цикл
		...
		Если Найти(СтрДетали.Имя, "ПрисоединенныеФайлы") > 0 
			...
			Или Найти(СтрДетали.Имя, "НаборыЗначений") > 0 Тогда
			КоэффициентТипа = КоэффициентТипа * 0.1;
		КонецЕсли;
		...
		Для каждого СтрПричины Из МассивПричин Цикл
			Если Лев(СтрПричины, 6) = "токен:" Тогда
				...
				// "документ" - слишком частое слово в 1С, оно дает шум
				Если ТекТокен = "документ" Или ТекТокен = "s:документ" Тогда
					Продолжить;
				КонецЕсли;
		...
		Если КоличествоТокенов > 1 Тогда
			...
			Бонус = Бонус + (Pow(КоличествоТокенов, 2) * 100); 
		КонецЕсли;
		...
		Для каждого Т Из ТокеныЗапроса Цикл
			...
			Если Т = "документ" Или СтрДлина(Т) < 3 Тогда Продолжить; КонецЕсли;
		...
		Если ЗначимыеТокены.Количество() > 1 И КоличествоСовпавших = ЗначимыеТокены.Количество() Тогда
			Бонус = Бонус + 1000; // Гигантский бонус, если ВСЕ слова запроса найдены в объекте
