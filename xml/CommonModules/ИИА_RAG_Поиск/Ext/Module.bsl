...
Функция ВыполнитьRerankИОграничение(Знач Результаты, Знач TopK = 10, Знач Контекст = Неопределено, Знач СсылкаДиалога = Неопределено) Экспорт
	...
	// Гибридный rerank через Yandex Embeddings (если доступны)
	Попытка
		Если ИИА_RAG_Эмбеддинги.ЭмбеддингиДоступны() И Результаты.Количество() > 0 И Контекст <> Неопределено Тогда
			ТекстЗапроса = "";
			Если ТипЗнч(Контекст) = Тип("Строка") Тогда
				ТекстЗапроса = Контекст;
			ИначеЕсли ТипЗнч(Контекст) = Тип("Структура") И Контекст.Свойство("ТекстЗапроса") Тогда
				ТекстЗапроса = Контекст.ТекстЗапроса;
			КонецЕсли;

			Если НЕ ПустаяСтрока(ТекстЗапроса) Тогда
				// Готовим кандидатов для rerank (берём top-20 по TF-IDF)
				ЛимитRerank = Мин(Результаты.Количество(), 20);
				МассивКандидатов = Новый Массив;
				Для Инд = 0 По ЛимитRerank - 1 Цикл
					Кандидат = Новый Структура("КлючЧанка,Скор", Результаты[Инд].Key, Результаты[Инд].Score);
					МассивКандидатов.Добавить(Кандидат);
				КонецЦикла;

				РезультатыRerank = ИИА_RAG_Эмбеддинги.RerankПоЭмбеддингам(ТекстЗапроса, МассивКандидатов, TopK);

				Если РезультатыRerank.Количество() > 0 Тогда
					// Пересортируем основную таблицу по результатам rerank
					Для Каждого ЭлементRerank Из РезультатыRerank Цикл
						Для Каждого СтрокаРезультатов Из Результаты Цикл
							Если СтрокаРезультатов.Key = ЭлементRerank.КлючЧанка Тогда
								СтрокаРезультатов.Score = СтрокаРезультатов.Score + ЭлементRerank.СкорЭмбеддинга * 10000;
								Прервать;
							КонецЕсли;
						КонецЦикла;
					КонецЦикла;
					Результаты.Сортировать("Score Убыв");
					ИИА_Сервер.ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[RAG_RERANK] Применён Yandex Embeddings rerank для " + Строка(РезультатыRerank.Количество()) + " кандидатов");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Исключение
		ИИА_Сервер.ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, "[RAG_RERANK] Ошибка Embeddings rerank: " + ОписаниеОшибки());
	КонецПопытки;
