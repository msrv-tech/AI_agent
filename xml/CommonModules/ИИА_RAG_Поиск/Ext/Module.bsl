...
// Предназначено для вызова через HTTP-bridge (automation/rag/rag_search.py).
//
// Параметры:
...
// Предназначено для вызова через HTTP-bridge (automation/rag/rag_search.py --fields).
//
// Параметры:
...
Функция ВыполнитьПоиск(Знач ЗапросТекст, Знач TopK = 10, Знач Контекст = Неопределено, Знач СсылкаДиалога = Неопределено) Экспорт
	...
	// 2. Пессимизация технического мусора
		...
		ОбъектРегистрОстатков = (СтрДетали.Тип = "AccumReg")
			И (Найти(НормИмя, "запас") > 0 Или Найти(НормИмя, "остат") > 0 Или Найти(НормИмя, "склад") > 0 Или Найти(НормИмя, "товар") > 0)
			И Найти(НормИмя, "прослежива") = 0;
		Если ЗапросПРОстатки И ОбъектРегистрОстатков Тогда
			Бонус = Бонус + 3500;
			Причины[Ключ].Добавить("ядро:остатки");
		КонецЕсли;
		Если ЗапросПРОстатки И СтрДетали.Тип = "Document" Тогда
			КоэффициентТипа = КоэффициентТипа * 0.25;
			Причины[Ключ].Добавить("пессимизация:документ_для_остатков");
		КонецЕсли;
		
		ЗапросПроЗаказ = Найти(ЗапросНорм, "ЗАКАЗ") > 0;
		ОбъектДокументЗаказа = (СтрДетали.Тип = "Document") И Найти(НормИмя, "заказ") > 0;
		ОбъектДокументРеализации = (СтрДетали.Тип = "Document")
			И (Найти(НормИмя, "реализац") > 0 Или Найти(НормИмя, "отгруз") > 0)
			И Найти(НормИмя, "заказ") = 0;
		Если ЗапросПроЗаказ И ОбъектДокументЗаказа Тогда
			Бонус = Бонус + 3500;
			Причины[Ключ].Добавить("ядро:заказ");
			Если (Найти(ЗапросНорм, "КЛИЕНТ") > 0 Или Найти(ЗапросНорм, "ПОКУПАТЕЛ") > 0)
				И (Найти(НормИмя, "клиент") > 0 Или Найти(НормИмя, "покупател") > 0
					Или Найти(НормСиноним, "клиент") > 0 Или Найти(НормСиноним, "покупател") > 0) Тогда
				Бонус = Бонус + 1500;
				Причины[Ключ].Добавить("ядро:заказ_клиента");
			КонецЕсли;
			Если Найти(НормИмя, "поставщик") > 0 Или Найти(НормСиноним, "поставщик") > 0
				Или Найти(НормИмя, "эмис") > 0 Или Найти(НормСиноним, "эмис") > 0
				Или Найти(НормИмя, "маркир") > 0 Или Найти(НормСиноним, "маркир") > 0 Тогда
				КоэффициентТипа = КоэффициентТипа * 0.25;
				Причины[Ключ].Добавить("пессимизация:заказ_не_клиента");
			КонецЕсли;
		КонецЕсли;
		Если ЗапросПроЗаказ И ОбъектДокументРеализации И Найти(ЗапросНорм, "РЕАЛИЗАЦ") = 0 И Найти(ЗапросНорм, "ПРОДАЖ") = 0 Тогда
			КоэффициентТипа = КоэффициентТипа * 0.25;
			Причины[Ключ].Добавить("пессимизация:реализация_для_заказа");
		КонецЕсли;
