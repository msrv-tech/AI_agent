...
Функция ВыполнитьПоиск(Знач ЗапросТекст, Знач TopK = 10, Знач Контекст = Неопределено) Экспорт
	...
	Для каждого СтрДетали Из ДеталиЧанков Цикл
		...
		Если СтрДетали.Тип = "Document" Тогда
			КоэффициентТипа = 1.5; // Документы важнее всего
		...
		ИначеЕсли СтрДетали.Тип = "Enum" Тогда
			КоэффициентТипа = 0.1; // Перечисления еще сильнее понижаем
		КонецЕсли;
		...
		Если КоличествоТокенов > 1 Тогда
			...
			Бонус = Бонус + (Pow(КоличествоТокенов, 2) * 50); 
		КонецЕсли;
		
		// 4. Бонус за точное совпадение фразы в имени или синониме
		...
		Если Найти(НормИмя, ЧистыйЗапрос) > 0 Тогда
			Бонус = Бонус + 300; // Увеличиваем бонус за точное попадание фразы
