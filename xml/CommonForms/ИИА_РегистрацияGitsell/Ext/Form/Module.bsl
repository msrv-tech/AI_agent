...
Процедура НачатьDeviceFlow()
	...
	Иначе
		...
		Если РасширениеВБезопасномРежимеНаСервере() Тогда
			СтрокаСтатуса = "Нет доступа к gitsell.ru. Проверьте разрешения расширения.";
		Иначе
...
&НаКлиенте
Процедура ОбновитьВидимостьРезультатаАвторизации()
	
	Элементы.СтрокаСтатуса.Видимость = НЕ ПустаяСтрока(СтрокаСтатуса);
	Элементы.UserCode.Видимость = НЕ ПустаяСтрока(UserCode);
	Элементы.VerificationUri.Видимость = НЕ ПустаяСтрока(VerificationUri);
	Элементы.EmailПользователя.Видимость = НЕ ПустаяСтрока(EmailПользователя);
