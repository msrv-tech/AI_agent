...
Процедура КомандаОтправить(Команда)
	...
	Попытка
		...
		Если Результат <> Неопределено Тогда
			...
			ЭтотОбъект.ПотреченоТокенов = ПотраченоВсего;
...
Процедура СпланироватьСледующиеШаги()
	...
	ЭтотОбъект.ПотреченоТокенов = ОбщееКоличествоТокенов;
...
Процедура ОбновитьПотреченоТокеновНаФорме()
	...
	Если ТекущийДиалог = Неопределено Или ТекущийДиалог.Пустая() Тогда
		ЭтотОбъект.ПотреченоТокенов = 0;
		Возврат;
	КонецЕсли;
	
	Попытка
		ЭтотОбъект.ПотреченоТокенов = ИИА_ВызовСервера.ПолучитьОбщееКоличествоТокенов(ТекущийДиалог);
	Исключение
