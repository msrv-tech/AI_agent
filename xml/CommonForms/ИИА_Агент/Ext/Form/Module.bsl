...
Процедура КомандаОтправить(Команда)
	...
	Попытка
		...
		// Запускаем серверный оркестратор
		ИИА_ВызовСервера.ОркестраторЗапустить(ТекущийДиалог);
...
Процедура ПриПриемеСообщения(ДанныеСообщения, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДанныеСообщения.Свойство("Диалог") И ДанныеСообщения.Диалог = ТекущийДиалог Тогда
		...
		// Обработка остановки оркестратора
		Если ДанныеСообщения.Свойство("ТипСобытия")
			И (ДанныеСообщения.ТипСобытия = "ОстановленПользователем"
				ИЛИ ДанныеСообщения.ТипСобытия = "ОшибкаПлана") Тогда
			
			ОстановитьОркестратор();
			ОбновитьСообщения();
			ОбновитьПланНаФорме();
			
			ТекстСобытия = ?(ДанныеСообщения.ТипСобытия = "ОшибкаПлана", "Ошибка создания плана", "Оркестратор остановлен");
			Состояние("ИИ Агент: " + ТекстСобытия,, ТекстСобытия);
			Возврат;
		КонецЕсли;
		
		// Для остальных уведомлений просто обновляем интерфейс
		ОбновитьСообщения();
		ОбновитьПланНаФорме();
		...
		Если ДанныеСообщения.Свойство("ТипСобытия") Тогда
			...
			ИначеЕсли ДанныеСообщения.ТипСобытия = "ОстановленПользователем" Тогда
				ТекстСобытия = "Оркестратор остановлен";
			ИначеЕсли ДанныеСообщения.ТипСобытия = "ОшибкаПлана" Тогда
