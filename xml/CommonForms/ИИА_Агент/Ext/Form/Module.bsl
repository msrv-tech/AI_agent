...
Процедура ОбновитьИнтерфейсЧатаНаСервере(Сообщения)
	...
	Для Индекс = 0 По Сообщения.Количество() - 1 Цикл
		...
		// Резюме всегда показываем — не фильтруем даже при наличии технических подстрок от ИИ
		Если НЕ СтрНачинаетсяС(ТекстСообщения, "=== РЕЗЮМЕ") Тогда
