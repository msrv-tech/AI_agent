...
Процедура ОбновитьИнтерфейсЧатаНаСервере(Сообщения)
	...
	Для Индекс = 0 По Сообщения.Количество() - 1 Цикл
		...
		// Пропускаем технические JSON-сообщения с планом в интерфейсе агента
		Если СтрНайти(СтрокаСообщения.Текст, """updated_plan"":") > 0 
			И СтрНайти(СтрокаСообщения.Текст, """next_step_dsl"":") > 0 Тогда
			Продолжить;
		КонецЕсли;
		
		// Пропускаем служебные сообщения оркестратора
		Если СтрНайти(СтрокаСообщения.Текст, "Следующий шаг выполнения по плану") > 0 
			ИЛИ СтрНайти(СтрокаСообщения.Текст, "DSL-сценарий выполнен успешно.") > 0 Тогда
			Продолжить;
		КонецЕсли;
