...
Процедура КомандаОтправить(Команда)
	...
	Если ОркестраторРаботает Тогда
		// Явная причина остановки — пользователь нажал кнопку "Остановить"
		ДобавитьВЛог("Система", "Остановка оркестратора: нажата кнопка 'Остановить' (пользователь)");
		ОстановитьОркестратор();
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Если Результат <> Неопределено Тогда
			...
			КонецЕсли;
				...
				КонецЕсли;
			// Всегда фиксируем и логируем токены по диалогу при каждом ответе ИИ
			ПотраченоВсего = 0;
			Если ЗначениеЗаполнено(ТекущийДиалог) Тогда
				Попытка
					ПотраченоВсего = ИИА_ВызовСервера.ПолучитьОбщееКоличествоТокенов(ТекущийДиалог);
				Исключение
					ДобавитьВЛог("Ошибка", "Не удалось получить ПотреченоТокенов: " + ОписаниеОшибки());
				КонецПопытки;
			КонецЕсли;
			
			ЭтотОбъект.ПотраченоТокенов = ПотраченоВсего;
			
			ДанныеUsage = Новый Структура("ПотреченоТокенов", ПотраченоВсего);
		...
		// Запускаем планирование следующих шагов (теперь через уведомления)
		// ПодключитьОбработчикПланирования(); // Больше не нужно, сервер пришлет уведомление
		...
		Если Результат <> Неопределено Тогда
			Если Результат.Свойство("Usage") И Результат.Usage <> Неопределено Тогда
				Если Результат.Usage.Свойство("ОстатокКредитов") И Результат.Usage.ОстатокКредитов > 0 Тогда
					...
					Если ПотраченоВсего > 0 Тогда
						СтрокаСтатуса = СтрокаСтатуса + ". Потрачено токенов: " + Формат(ПотраченоВсего, "ЧН=0; ЧГ=");
					КонецЕсли;
				ИначеЕсли Результат.Usage.Свойство("TotalTokens") Тогда
					СтрокаСтатуса = "Потрачено токенов: " + Формат(ПотраченоВсего, "ЧН=0; ЧГ=");
					СтрокаСтатуса = СтрокаСтатуса + " (последний вызов: " + Формат(Результат.Usage.TotalTokens, "ЧН=0; ЧГ=") + ")";
				Иначе
...
Процедура СпланироватьСледующиеШаги()
	...
	// Отображаем в реквизите формы (добавлен пользователем)
	ЭтотОбъект.ПотраченоТокенов = ОбщееКоличествоТокенов;
	...
	Если ПланДанные.Количество() = 0 Тогда
		...
		ПараметрыЗадания.Добавить(ТекущийДиалог);
		ИИА_ВызовСервера.ЗапуститьФоновоеЗадание("ИИА_Сервер.ИнициализироватьПлан", ПараметрыЗадания, "ИИ_ИнициализацияПлана_" + Строка(ТекущийДиалог.УникальныйИдентификатор()));
		
		// ФоновыеЗадания.Выполнить("ИИА_Сервер.ИнициализироватьПлан", ПараметрыЗадания, , "ИИ_ИнициализацияПлана_" + Строка(ТекущийДиалог.УникальныйИдентификатор()));
		
		// УспехПланирования = ИИА_ВызовСервера.ИнициализироватьПлан(ТекущийДиалог); // УДАЛЕНО: теперь в фоне
	...
	Если ИИА_ВызовСервера.ПланЗавершен(ТекущийДиалог) Тогда
		...
		// ЗАПУСКАЕМ В ФОНЕ (т.к. проверка тоже вызывает ИИ)
		ПараметрыЗадания = Новый Массив;
		ПараметрыЗадания.Добавить(ТекущийДиалог);
		ИИА_ВызовСервера.ЗапуститьФоновоеЗадание("ИИА_Сервер.ВыполнитьФинальнуюПроверкуИSummary", ПараметрыЗадания, "ИИ_Финал_" + Строка(ТекущийДиалог.УникальныйИдентификатор()));
		
		// ФоновыеЗадания.Выполнить("ИИА_Сервер.ВыполнитьФинальнуюПроверкуИSummary", ПараметрыЗадания, , "ИИ_Финал_" + Строка(ТекущийДиалог.УникальныйИдентификатор()));
		...
		// 
		// // Для режима "Запрос1С" всегда пытаемся показать ТабличныйДокумент в конце работы
		// ПопытатьсяПоказатьТабличныйДокумент(Истина);
		// 
	...
	// ЗАПУСКАЕМ В ФОНЕ (чтобы не блокировать интерфейс)
	ПараметрыЗадания = Новый Массив;
	ПараметрыЗадания.Добавить(ТекущийДиалог);
	
	ИИА_ВызовСервера.ЗапуститьФоновоеЗадание("ИИА_Сервер.СгенерироватьПродолжениеВыполнения", ПараметрыЗадания, "ИИ_ГенерацияШага_" + Строка(ТекущийДиалог.УникальныйИдентификатор()));
	
	// ФоновыеЗадания.Выполнить("ИИА_Сервер.СгенерироватьПродолжениеВыполнения", ПараметрыЗадания, , "ИИ_ГенерацияШага_" + Строка(ТекущийДиалог.УникальныйИдентификатор()));
	
	// ИИА_ВызовСервера.СгенерироватьПродолжениеВыполнения(ТекущийДиалог); // УДАЛЕНО: теперь в фоне
	
	// Если в процессе был выполнен RunQuery — покажем ТабличныйДокумент (один раз, по флагу в хранилище)
	ПопытатьсяПоказатьТабличныйДокумент();
	
	ОбновитьПланНаФорме();
	ОбновитьСообщения();
	// ПодключитьОбработчикПланирования(); // Больше не нужно, сервер пришлет уведомление по завершении фонового задания
...
Процедура ПриПриемеСообщения(ДанныеСообщения, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДанныеСообщения.Свойство("Диалог") И ДанныеСообщения.Диалог = ТекущийДиалог Тогда
		...
		// Если задача завершена, останавливаем оркестратор
		Если ДанныеСообщения.Свойство("ТипСобытия") И ДанныеСообщения.ТипСобытия = "ЗадачаЗавершена" Тогда
			ОстановитьОркестратор();
			ОбновитьСообщения();
			ОбновитьПланНаФорме();
		...
		// Если оркестратор работает, запускаем следующий шаг
		Если ОркестраторРаботает Тогда
			// Используем обработчик ожидания с минимальным интервалом, 
			// чтобы разорвать контекст выполнения и не блокировать интерфейс
			ПодключитьОбработчикОжидания("СпланироватьСледующиеШаги", 0.1, Истина);
		ИначеЕсли ДанныеСообщения.Свойство("ТипСобытия") И ДанныеСообщения.ТипСобытия = "ЗапросПринят" Тогда
			// Если пришел новый запрос, а оркестратор еще не запущен (или был остановлен),
			// запускаем его и планируем первый шаг
			ЗапуститьОркестратор();
			ПодключитьОбработчикОжидания("СпланироватьСледующиеШаги", 0.1, Истина);
		Иначе
			// Если оркестратор не запущен, просто обновляем интерфейс
			ОбновитьСообщения();
			ОбновитьПланНаФорме();
		КонецЕсли;
		
		// Оповещаем пользователя (опционально)
		Если ДанныеСообщения.Свойство("ТипСобытия") Тогда
			ТекстСобытия = "Событие";
			Если ДанныеСообщения.ТипСобытия = "ПланИнициализирован" Тогда
				ТекстСобытия = "План создан";
			ИначеЕсли ДанныеСообщения.ТипСобытия = "ШагВыполнен" Тогда
				ТекстСобытия = "Шаг выполнен";
			ИначеЕсли ДанныеСообщения.ТипСобытия = "ЗапросПринят" Тогда
				ТекстСобытия = "Запрос принят";
			КонецЕсли;
			Состояние("ИИ Агент: " + ТекстСобытия,, ТекстСобытия);
		КонецЕсли;
...
Процедура ПопытатьсяПоказатьТабличныйДокумент(ЛогироватьПропуск = Ложь)
	...
	Попытка
		...
		Если ДанныеДляТаблицы = Неопределено Тогда
			Если ЛогироватьПропуск Тогда
				ДобавитьВЛог("ТабличныйДокумент", "Пропуск: нет непоказанных данных в хранилище");
			КонецЕсли;
			Возврат;
		КонецЕсли;
	...
	Исключение
		...
		ДобавитьВЛог("Ошибка", "Ошибка при получении данных запроса для ТабличногоДокумента: " + ОписаниеОшибки());
	КонецПопытки;
...
Процедура ОбновитьИнтерфейсЧатаНаСервере(Сообщения)
	...
	Для Индекс = 0 По Сообщения.Количество() - 1 Цикл
		...
		Если СтрНайти(ТекстСообщения, """updated_plan"":") > 0 
			ИЛИ СтрНайти(ТекстСообщения, """next_step_dsl"":") > 0
			ИЛИ СтрНайти(ТекстСообщения, """kind"": ""dsl_system_result""") > 0
			ИЛИ СтрНайти(ТекстСообщения, """changed_objects"":") > 0
			ИЛИ (СтрНачинаетсяС(СокрЛП(ТекстСообщения), "[") И Прав(СокрЛП(ТекстСообщения), 1) = "]") Тогда
			Продолжить;
		КонецЕсли;
		
		// Пропускаем служебные сообщения оркестратора
		Если СтрНайти(ТекстСообщения, "Следующий шаг выполнения по плану") > 0 
			ИЛИ СтрНайти(ТекстСообщения, "DSL-сценарий выполнен успешно.") > 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	// Убедимся, что родительская группа тоже растягивается
	Попытка
		Элементы.ГруппаТело.РастягиватьПоВертикали = Истина;
		Элементы.ГруппаТело.РастягиватьПоГоризонтали = Истина;
	Исключение
		// Если элемента нет на форме (например, в другой версии формы), игнорируем
	КонецПопытки;
	
	// Для того чтобы подчиненные элементы (строки чата) растягивались, 
	// установим ширину группы в 0 (авто) и убедимся, что контейнеры строк тоже растягиваются.
	ГруппаЧат.Ширина = 0; 
	...
	Для Индекс = 0 По Сообщения.Количество() - 1 Цикл
		...
		ГруппаПузырь.Ширина = 0; // Автоматическая ширина с растягиванием
		
		// Если это сообщение пользователя, оно прижато к правому краю, но может растягиваться.
		// Если это ИИ/Система, оно занимает всю ширину (распорки справа нет).
		...
		// УДАЛЕНО: Распорка справа для сообщений ИИ больше не нужна, 
		// чтобы сообщения занимали всю ширину.
...
Процедура ОбновитьПотреченоТокеновНаФорме()
	...
	Исключение
		ДобавитьВЛог("Ошибка", "Не удалось обновить ПотреченоТокенов на форме: " + ОписаниеОшибки());
	КонецПопытки;
...
Процедура СоздатьИОткрытьТабличныйДокумент(ДанныеЗапроса)
	...
	Если ДанныеЗапроса = Неопределено Или ТипЗнч(ДанныеЗапроса) <> Тип("Массив") Или ДанныеЗапроса.Количество() = 0 Тогда
		ДобавитьВЛог("ТабличныйДокумент", "Не показан: нет данных (пусто/неверный тип)");
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Если ТабличныйДокумент <> Неопределено Тогда
			...
			ДобавитьВЛог("ТабличныйДокумент", "Показан (строк=" + Формат(ДанныеЗапроса.Количество(), "ЧН=0") + ")");
			...
			КонецЕсли;
		Иначе
			ДобавитьВЛог("ТабличныйДокумент", "Не показан: сервер вернул Неопределено при создании");
		КонецЕсли;
		
	Исключение
		Сообщить("Ошибка при создании табличного документа: " + ОписаниеОшибки());
		ДобавитьВЛог("Ошибка", "Ошибка при создании табличного документа: " + ОписаниеОшибки());
	КонецПопытки;
