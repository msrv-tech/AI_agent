	...
	Если ДанныеСообщения.Свойство("Диалог") И ДанныеСообщения.Диалог = ТекущийДиалог Тогда
		...
		Если ДанныеСообщения.Свойство("ТипСобытия") Тогда
			...
			Если ТипСобытия = "ЗадачаЗавершена" ИЛИ ТипСобытия = "ОстановленПользователем" 
				...
				// Повторное обновление через 2 с и 4 с - на случай гонки с записью резюме в фоновом задании
				Если ТипСобытия = "ЗадачаЗавершена" Тогда
					ПодключитьОбработчикОжидания("ПовторноеОбновлениеДиалогаПоОжиданию", 2, Истина);
					ПодключитьОбработчикОжидания("ПовторноеОбновлениеДиалогаПоОжиданию", 4, Истина);
				КонецЕсли;
...
&НаКлиенте
// Вызывается через ПодключитьОбработчикОжидания при «ЗадачаЗавершена» для повторной загрузки резюме и ссылок на объекты
Процедура ПовторноеОбновлениеДиалогаПоОжиданию()
	Если ТекущийДиалог = Неопределено ИЛИ ТекущийДиалог.Пустая() Тогда
		Возврат;
	КонецЕсли;
	ОбновитьОтображениеДиалога();
...
Процедура ОбновитьИнтерфейсЧатаНаСервере(Сообщения)
	...
	Для Индекс = 0 По Сообщения.Количество() - 1 Цикл
		...
		Если ЭлементДляПрокруткиКНовейшему = Неопределено Тогда
			ЭлементДляПрокруткиКНовейшему = ГруппаПузырь;
		КонецЕсли;
		...
		Исключение
			// Игнорируем - свойство может отсутствовать в старых версиях
...
Процедура КомандаВверх(Команда)
	...
	Если Количество > 0 Тогда
		// Ищем последний существующий элемент (индекс может быть пропущен)
		Индекс = Количество - 1;
		Пока Индекс >= 0 Цикл
			Элемент = Элементы.Найти("дин_msg_" + Формат(Индекс, "ЧГ=0") + "_Group");
...
Процедура КомандаВниз(Команда)
	...
	// Прокрутка к НИЗУ (к старым сообщениям); ищем первый существующий элемент
	СообщенияЧат = ИИА_Клиент.ЗагрузитьСообщенияДиалога(ТекущийДиалог);
	Количество = СообщенияЧат.Количество();
	Если Количество > 0 Тогда
