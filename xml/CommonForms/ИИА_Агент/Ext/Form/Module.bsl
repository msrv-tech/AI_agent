...
Процедура ОбновитьИнтерфейсЧатаНаСервере(Сообщения)
	...
	Для Индекс = 0 По Сообщения.Количество() - 1 Цикл
		...
		// Резюме никогда не фильтруем
		Если СтрНачинаетсяС(ТекстСообщения, "=== РЕЗЮМЕ") Тогда
			// Показываем
		ИначеЕсли (СтрНайти(ТекстСообщения, """updated_plan"":") > 0 И СтрНайти(ТекстСообщения, """next_step_dsl"":") > 0)
	...
	Для Индекс = 0 По Сообщения.Количество() - 1 Цикл
		...
		ГруппаПузырь.Ширина = 50; // Автоматическая ширина с растягиванием  
