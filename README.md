...
## 🏆 Хакатон AI Sprint 2026 — Кейс 11

Этот форк подготовлен для **AI Sprint 2026, Кейс 11: Агент-консультант менеджера по продажам**.

### Реализованные сценарии

**Обязательные:**
1. Актуальные цены по позициям из прайс-листа
2. История продаж клиенту за период с расчётом маржинальности
...
6. Сводный отчёт по клиенту (мульти-источник: цены + продажи + задолженность)

**Овер-бонус:**
7. Семантический поиск через Yandex Embeddings (гибридный TF-IDF + cosine similarity rerank)

### Ключевые доработки

| Доработка | Описание |
|-----------|----------|
| Динамический доменный промпт | Автоматическая генерация из метаданных конфигурации (ERP 2, УТ 11 и др.) |
| RAG-синонимы | 10 доменных синонимов (дебиторка, маржа, прайс и др.) |
| HTTP-провайдер | Настраиваемый порт и SSL (поддержка LM Studio) |
| Yandex Embeddings | Модуль + регистр хранения векторов + гибридный rerank |
| DSL-расширение | ImportPriceList, ParseFileForContractor |
| Тесты | 4 сценария case11_* в automation/test_examples.py |
...
**Кейс 11 — примеры для менеджера по продажам:**
*   *"Покажи текущую цену на Парацетамол по всем видам цен."*
*   *"Покажи историю продаж клиенту ФармаПлюс за последние 3 месяца с расчетом маржинальности."*
*   *"Какая дебиторская задолженность у клиента ФармаПлюс? Есть ли просроченные платежи?"*
*   *"Сводка по клиенту ФармаПлюс: текущие цены на его товары, продажи за квартал, задолженность."*
*   *"Загрузи прайс-лист из файла /tmp/prices.xlsx с видом цены Оптовая."*
*   *"Создай карточку контрагента из файла /tmp/company.pdf."*
